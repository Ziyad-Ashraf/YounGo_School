...
$youngo_wishlist_home_url = function_exists('youngo_frontend_home_url') ? youngo_frontend_home_url($youngo_frontend_language) : site_url('home');
$youngo_wishlist_courses_url = function_exists('youngo_frontend_courses_url') ? youngo_frontend_courses_url($youngo_frontend_language) : site_url('home/courses');
$youngo_wishlist_login_url = function_exists('youngo_frontend_login_url') ? youngo_frontend_login_url($youngo_frontend_language) : site_url('login');
$youngo_wishlist_contact_url = function_exists('youngo_frontend_contact_url') ? youngo_frontend_contact_url($youngo_frontend_language) : site_url('home/contact_us');
$is_logged_in = $this->session->userdata('user_login') == true;
...
if ($is_logged_in) {
    if (isset($my_courses) && is_array($my_courses)) {
        $wishlist_courses = $my_courses;
    } else {
        $wishlist_courses = $this->crud_model->get_courses_by_wishlists();
    }
    if (function_exists('youngo_frontend_translate_course_rows')) {
        $wishlist_courses = youngo_frontend_translate_course_rows($wishlist_courses, $youngo_frontend_language);
    }
    ...
    if ($user_id = $this->session->userdata('user_id')) {
        ...
        if ($wishlist != '') {
            ...
            $wishlist_ids = is_array($decoded_wishlist) ? $decoded_wishlist : array();
        }
    }
}
?>
...
<section class="youngo-commerce-hero">
    <div class="youngo-container">
        <nav class="youngo-breadcrumb" aria-label="<?php echo youngo_wishlist_phrase('breadcrumb', '', $youngo_frontend_language); ?>">
            <a href="<?php echo $youngo_wishlist_home_url; ?>"><?php echo youngo_wishlist_phrase('home', '', $youngo_frontend_language); ?></a>
            <span>/</span>
            <span><?php echo youngo_wishlist_phrase('my_wishlist', '', $youngo_frontend_language); ?></span>
        </nav>

        <div class="youngo-commerce-hero__grid">
            <div>
                <p class="youngo-eyebrow"><?php echo youngo_wishlist_phrase('saved_courses', '', $youngo_frontend_language); ?></p>
                <h1><?php echo youngo_wishlist_phrase('my_wishlist', '', $youngo_frontend_language); ?></h1>
                <p><?php echo youngo_wishlist_phrase('keep_favorite_youngo_courses_in_one_place_then_return_when_your_child_is_ready_to_start.', '', $youngo_frontend_language); ?></p>
            </div>

            <aside class="youngo-commerce-summary-card" aria-label="<?php echo youngo_wishlist_phrase('wishlist_summary', '', $youngo_frontend_language); ?>">
                <span><?php echo youngo_wishlist_phrase('saved_courses', '', $youngo_frontend_language); ?></span>
                <strong><?php echo count($wishlist_ids); ?></strong>
                <small><?php echo youngo_wishlist_phrase('saved_courses_stay_here_so_you_can_compare_learning_paths_before_access_is_granted.', '', $youngo_frontend_language); ?></small>
            </aside>
        </div>
    </div>
</section>
    ...
    <div class="youngo-container">
        <?php if (!$is_logged_in): ?>
            <div class="youngo-commerce-empty">
                ...
                <p class="youngo-eyebrow"><?php echo youngo_wishlist_phrase('sign_in_required', '', $youngo_frontend_language); ?></p>
                <h2><?php echo youngo_wishlist_phrase('log_in_to_view_your_wishlist', '', $youngo_frontend_language); ?></h2>
                <p><?php echo youngo_wishlist_phrase('wishlist_items_are_saved_to_your_learner_account_so_they_stay_available_across_visits.', '', $youngo_frontend_language); ?></p>
                <div class="youngo-commerce-empty__actions">
                    <a class="youngo-button" href="<?php echo $youngo_wishlist_login_url; ?>"><?php echo youngo_wishlist_phrase('log_in', '', $youngo_frontend_language); ?></a>
                    <a class="youngo-button youngo-button--secondary" href="<?php echo $youngo_wishlist_courses_url; ?>"><?php echo youngo_wishlist_phrase('browse_courses', '', $youngo_frontend_language); ?></a>
                </div>
            </div>
        <?php elseif (count($wishlist_courses) > 0): ?>
            <div class="youngo-commerce-toolbar">
                <div>
                    <p class="youngo-eyebrow"><?php echo youngo_wishlist_phrase('wishlist', '', $youngo_frontend_language); ?></p>
                    <h2><?php echo youngo_wishlist_phrase('courses_you_saved', '', $youngo_frontend_language); ?></h2>
                </div>
                <a class="youngo-button youngo-button--secondary" href="<?php echo $youngo_wishlist_courses_url; ?>"><?php echo youngo_wishlist_phrase('explore_more_courses', '', $youngo_frontend_language); ?></a>
            </div>

            <div class="youngo-wishlist-grid" id="wishlistItems">
                <?php foreach ($wishlist_courses as $course): ?>
                    <?php
                    $course_id = (int) $course['id'];
                    $course_url = function_exists('youngo_frontend_course_detail_url') ? youngo_frontend_course_detail_url($course, $youngo_frontend_language) : site_url('home/course/' . rawurlencode(slugify($course['title'])) . '/' . $course_id);
                    ...
                    $wishlist_access_mode = isset($course['youngo_access_mode']) ? (string) $course['youngo_access_mode'] : '';
                    $price_label = $wishlist_access_mode === 'subscription_only' && empty($course['is_free_course'])
                        ? youngo_wishlist_phrase('subscription_access', '', $youngo_frontend_language)
                        : ($course['is_free_course'] ? youngo_wishlist_phrase('free', '', $youngo_frontend_language) : ($course['discount_flag'] ? currency($course['discounted_price']) : currency($course['price'])));
                    $youngo_boundary_state = youngo_wishlist_course_boundary_state($course, (int) $this->session->userdata('user_id'));
                    $wishlist_checkout_cta_decision = array('show_cta' => false, 'target_url' => null, 'label_text' => '');
                    if (function_exists('youngo_checkout_cta_decision')) {
                        try {
                            $wishlist_checkout_cta_decision = youngo_checkout_cta_decision($course, (int) $this->session->userdata('user_id'), array(
                    ...
                    $wishlist_show_checkout_cta = !empty($wishlist_checkout_cta_decision['show_cta']) && !empty($wishlist_checkout_cta_decision['target_url']);
                    $wishlist_show_subscription_cta = $wishlist_access_mode === 'subscription_only' || $wishlist_access_mode === 'subscription_and_purchase';
                    $wishlist_checkout_label = !empty($wishlist_checkout_cta_decision['label_text']) ? $wishlist_checkout_cta_decision['label_text'] : youngo_wishlist_phrase('continue_to_checkout', 'Continue to checkout', $youngo_frontend_language);
                    ?>
                    <article class="youngo-commerce-course-card">
                        <a class="youngo-commerce-course-card__media" href="<?php echo $course_url; ?>">
                            <img loading="lazy" src="<?php echo $this->crud_model->get_course_thumbnail_url($course_id); ?>" alt="<?php echo htmlspecialchars($course['title']); ?>">
                        ...
                        <div class="youngo-commerce-course-card__body">
                            ...
                            <h3><a href="<?php echo $course_url; ?>"><?php echo htmlspecialchars($course['title']); ?></a></h3>
                            ...
                            <div class="youngo-commerce-course-card__actions">
                                ...
                                <?php elseif ($wishlist_show_checkout_cta): ?>
                                    <a class="youngo-button youngo-button--small" href="<?php echo html_escape($wishlist_checkout_cta_decision['target_url']); ?>" data-youngo-checkout-cta="local-wishlist"><i class="fa-solid fa-credit-card"></i> <?php echo htmlspecialchars($wishlist_checkout_label, ENT_QUOTES, 'UTF-8'); ?></a>
                                <?php elseif ($wishlist_show_subscription_cta): ?>
                                ...
                                <?php else: ?>
                                    <a class="youngo-button youngo-button--small" href="<?php echo $youngo_wishlist_contact_url; ?>"><?php echo youngo_wishlist_phrase('contact', '', $youngo_frontend_language); ?></a>
                                <?php endif; ?>
                            </div>
                        </div>
                    </article>
        ...
        <?php else: ?>
            <div class="youngo-commerce-empty" id="wishlistItems">
                <div class="youngo-commerce-empty__icon"><i class="fa-regular fa-heart"></i></div>
                ...
                <a class="youngo-button" href="<?php echo $youngo_wishlist_courses_url; ?>"><?php echo youngo_wishlist_phrase('browse_courses', '', $youngo_frontend_language); ?></a>
            </div>
        <?php endif; ?>
    </div>
</section>
