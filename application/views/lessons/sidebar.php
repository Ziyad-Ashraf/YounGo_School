<?php
  ...
  $is_restricted = 1;
  $total_sections = is_array($sections) ? count($sections) : 0;
...
<?php if(is_array($sections) && count($sections) > 0): ?>
  <div class="course-playing-sidebar">
    <div class="youngo-lesson-sidebar__head">
      <p class="youngo-sidebar-eyebrow"><i class="far fa-list-alt"></i> <?php echo youngo_lesson_phrase('learning_path', 'Learning path'); ?></p>
      <h4 class="title"><?php echo youngo_lesson_phrase('course_content', 'Course content'); ?></h4>
      <div class="youngo-lesson-sidebar__summary">
        <span><?php echo $total_sections . ' ' . youngo_lesson_phrase('sections', 'sections'); ?></span>
        <span><?php echo $total_lessons . ' ' . youngo_lesson_phrase('lessons', 'lessons'); ?></span>
        <span><?php echo $completed_count . ' ' . youngo_lesson_phrase('completed', 'completed'); ?></span>
      </div>
    </div>
    ...
    <div class="accordion custom-accordion" id="accordionContent">
      <?php foreach($sections as $key => $section): ?>
        ...
        <div class="accordion-item youngo-lesson-section">
          <h2 class="accordion-header" id="section<?php echo $section['id']; ?>">
            <button class="accordion-button <?php if($lesson_details['section_id'] != $section['id']) echo 'collapsed'; ?>" type="button" data-bs-toggle="collapse" data-bs-target="#collapseOne<?php echo $section['id'] ?>" aria-expanded="true" aria-controls="collapseOne<?php echo $section['id'] ?>">
              <div class="d-flex flex-column youngo-lesson-section-title">
            ...
            <div class="accordion-body position-relative">
              ...
              <ul class="course-content-items" style="<?php if($is_restricted) echo 'filter: blur(1px);' ?>">
                ...
                if (function_exists('youngo_frontend_translate_lesson_rows')) {
                  $lessons = youngo_frontend_translate_lesson_rows($lessons, $youngo_lesson_language);
                }
                foreach($lessons as $key => $lesson):
                  ...
                  <li class="item youngo-lesson-nav-item <?php if($lesson['id'] == $lesson_details['id']) echo 'active'; ?>">
                    <a href="<?php echo $lesson_url; ?>" class="d-flex align-items-baseline w-100 checkbox-box-a youngo-lesson-nav-link">
                      <?php if(in_array($lesson['id'], $completed_lessons)){
                        $chekbox = 'title="'.youngo_lesson_phrase('uncheck', 'Uncheck').'" data-bs-toggle="tooltip" checked';
                      }else{
                        $chekbox = 'title="'.youngo_lesson_phrase('mark_as_complete', 'Mark as complete').'" data-bs-toggle="tooltip"';
                      }?>
                      ...
                      <span class="mx-2 d-grid">
                        <span class="m-0 p-0 youngo-lesson-title-text"><?php echo $lesson['title']; ?></span>
                        <span class="lesson-icon">
                          <?php if($lesson['lesson_type'] == 'other' || $lesson['lesson_type'] == 'text'): ?>
                            <i class="far fa-file-alt me-1"></i>
                            <?php echo youngo_lesson_phrase($lesson['attachment_type'], $lesson['attachment_type']); ?>
                          <?php elseif($lesson['lesson_type'] == 'quiz'): ?>
                            <i class="far fa-question-circle me-1"></i><?php echo youngo_lesson_phrase('quiz', 'Quiz'); ?>
                          <?php elseif($lesson['lesson_type'] == 'audio'): ?>
                            <i class="far fa-file-audio me-1"></i><?php echo youngo_lesson_phrase('audio', 'Audio'); ?>
                          <?php else: ?>
                            <i class="far fa-file-video me-1"></i><?php echo youngo_lesson_phrase('video', 'Video'); ?>
                          <?php endif; ?>
                        </span>
                      </span>
                      <span class="ms-auto youngo-lesson-duration"><?php echo $lesson['duration']; ?></span>
                    </a>
                  </li>
