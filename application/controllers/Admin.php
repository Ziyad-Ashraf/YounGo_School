...
class Admin extends CI_Controller
{
    ...
    public function youngo_checkout_coupon_usage()
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if (!in_array($filter, array('all', 'zero_amount', 'course', 'subscription'), true)) {
            $filter = 'all';
        }
    ...
    public function user_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_user_form') {
            ...
            $page_data['can_view_youngo_entitlement_summary'] = $this->can_view_youngo_entitlement_summary();
            if ($page_data['can_view_youngo_entitlement_summary']) {
                $this->load->model('Youngo_entitlement_model', 'youngo_entitlement_model');
                $page_data['youngo_user_entitlement_summary'] = $this->youngo_entitlement_model->get_admin_user_entitlement_summary((int) $param2);
            }
            $this->load->view('backend/index', $page_data);
        }
    }
    ...
    public function frontend_settings($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'contact_info') {
            ...
            $this->session->set_flashdata('flash_message', 'Contact information updated successfully');
    ...
    public function youngo_homepage($param1 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'update') {
            $this->crud_model->update_youngo_homepage_content();
            $this->session->set_flashdata('flash_message', get_phrase('YounGo homepage content updated successfully'));
            redirect(site_url('admin/youngo_homepage'), 'refresh');
        }

        $page_data['page_name']  = 'youngo_homepage';
        $page_data['page_title'] = get_phrase('YounGo homepage');
        $this->load->view('backend/index', $page_data);
    }
    ...
    public function course_actions($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            $can_manage_youngo_course_access = $this->can_manage_youngo_course_access();
            $course_id = $this->crud_model->add_course("", array(
                'can_manage_youngo_course_access' => $can_manage_youngo_course_access,
            ));
            ...
            $can_manage_youngo_course_access = $this->can_manage_youngo_course_access();
            $course_update_status = $this->crud_model->update_course($param2, "", array(
                'can_manage_youngo_course_access' => $can_manage_youngo_course_access,
            ));

            if ($course_update_status === false) {
                redirect(site_url('admin/course_form/course_edit/' . $param2), 'refresh');
                return;
            }
    ...
    public function course_form($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == 'add_course') {
            ...
            $page_data['can_manage_youngo_course_access'] = $this->can_manage_youngo_course_access();
            $this->load->view('backend/index', $page_data);
            ...
            if ($page_data['can_view_youngo_entitlement_summary']) {
                $this->load->model('Youngo_entitlement_model', 'youngo_entitlement_model');
                $page_data['youngo_course_entitlement_summary'] = $this->youngo_entitlement_model->get_admin_course_entitlement_summary((int) $param2);
            }
            $this->load->view('backend/index', $page_data);
    ...
    private function can_manage_youngo_course_access()
    {
        if (file_exists(APPPATH . 'helpers/youngo_capability_helper.php')) {
            $this->load->helper('youngo_capability');
        }
        ...
        return youngo_user_has_capability($this->session->userdata('user_id'), 'manage_courses') ? true : false;
    }

    private function can_view_youngo_entitlement_summary()
    {
        if (file_exists(APPPATH . 'helpers/youngo_capability_helper.php')) {
            $this->load->helper('youngo_capability');
        }
    ...
    public function manage_language($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'add_language') {
            ...
            if ($this->youngo_is_blocked_ui_language_target($language)) {
                $this->session->set_flashdata('error_message', 'Arabic translated is not a UI language target. Use arabic as the canonical Arabic UI language.');
                redirect(site_url('admin/manage_language'), 'refresh');
            }
        ...
        $this->load->model('Youngo_language_phrase_model', 'youngo_language_phrase_model');
        $page_data['languages']  = $this->crud_model->get_all_languages();
        $page_data['youngo_edit_phrase_languages'] = $this->youngo_language_phrase_model->get_edit_phrase_language_options();
    ...
    public function update_phrase_with_ajax()
    {
        ...
        if (strtolower(trim((string) $current_editing_language)) === 'arabic') {
            $this->load->model('Youngo_language_phrase_model', 'youngo_language_phrase_model');
            $this->youngo_language_phrase_model->mark_phrase_manual_override($key, 'arabic', $this->youngo_current_user_id());
        }
        echo $current_editing_language . ' ' . $key . ' ' . $updatedValue;
    }

    public function youngo_arabic_pack_import_preview()
    {
        if ($this->youngo_request_method() !== 'GET') {
            $this->output
        ...
        check_permission('settings');
        $this->youngo_require_root_admin_for_language_preview();

        $mode = $this->input->get('mode', true);
        $this->load->model('Youngo_language_phrase_model', 'youngo_language_phrase_model');
        $preview = $this->youngo_language_phrase_model->preview_arabic_pack_import($mode);

        $this->output
            ->set_content_type('application/json')
            ->set_output(json_encode(array(
                'ok' => !empty($preview['valid']),
                'preview' => $preview,
                'message' => 'Arabic pack import preview only. Full import remains disabled.',
            )));
    }

    public function youngo_arabic_pack_import_apply()
    {
        if ($this->youngo_request_method() !== 'POST') {
            $this->output
                ->set_status_header(405)
                ->set_content_type('application/json')
                ->set_output(json_encode(array(
        ...
        if ($mode !== 'missing_blank_only') {
            $this->output
                ...
                ->set_output(json_encode(array(
                    'ok' => false,
                    'message' => 'Only missing_blank_only is enabled for safe Arabic import apply.',
                )));
            return;
        }

        $this->load->model('Youngo_language_phrase_model', 'youngo_language_phrase_model');
        $result = $this->youngo_language_phrase_model->apply_arabic_pack_import('missing_blank_only', $this->youngo_current_user_id(), false);

        $this->output
            ->set_content_type('application/json')
            ->set_output(json_encode(array(
                'ok' => !empty($result['applied']) && empty($result['blocked']),
                'result' => $result,
                'message' => 'Safe Arabic pack import apply completed for missing_blank_only.',
            )));
    }

    public function youngo_edit_phrase_paginated_data()
    {
        if ($this->youngo_request_method() !== 'GET') {
            $this->output
                ->set_status_header(405)
        ...
        $this->load->model('Youngo_language_phrase_model', 'youngo_language_phrase_model');
        $result = $this->youngo_language_phrase_model->get_paginated_edit_phrases($language, $page, $per_page, $search);
    ...
    protected function youngo_require_root_admin_for_language_preview()
    {
        if (file_exists(APPPATH . 'helpers/youngo_capability_helper.php')) {
            $this->load->helper('youngo_capability');
        }
        ...
        if (function_exists('youngo_is_root_admin')) {
            $is_root = youngo_is_root_admin($this->youngo_current_user_id());
        } elseif (function_exists('is_root_admin')) {
            $is_root = is_root_admin($this->youngo_current_user_id());
        }
    ...
    protected function youngo_current_user_id()
    {
        return (int) $this->session->userdata('user_id');
    }

    protected function youngo_request_method()
    {
    ...
    public function coupons($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            ...
            $response ? $this->session->set_flashdata('flash_message', get_phrase('coupon_added_successfully')) : $this->session->set_flashdata('error_message', 'Coupon could not be created. Use a unique code, a discount from 1% to 100%, and a start date on or before the expiry date.');
            ...
            $response ? $this->session->set_flashdata('flash_message', get_phrase('coupon_updated_successfully')) : $this->session->set_flashdata('error_message', 'Coupon could not be updated. Use a unique code, a discount from 1% to 100%, and a start date on or before the expiry date.');
            redirect(site_url('admin/coupons'), 'refresh');
        } elseif ($param1 == "delete") {
            // CHECK ACCESS PERMISSION
            check_permission('coupon');
    ...
    public function edit_blog_category($blog_category_id = "")
    {
        ...
        $data['youngo_blog_category_translations'] = method_exists($this->crud_model, 'youngo_get_blog_category_translations')
            ? $this->crud_model->youngo_get_blog_category_translations($blog_category_id)
            : array('english' => array(), 'arabic' => array());
    ...
    public function blog_category($param1 = "", $param2 = "")
    {
        if ($param1 == 'add') {
            ...
            if ($response == true) {
                $this->session->set_flashdata('flash_message', 'Blog category added successfully');
            } else {
                $this->session->set_flashdata('error_message', 'There is already a blog category with this name');
            }
            redirect(site_url('admin/blog_category'), 'refresh');
        } elseif ($param1 == 'update') {
            $response = $this->crud_model->update_blog_category($param2);
            if ($response == true) {
                $this->session->set_flashdata('flash_message', 'Blog category updated successfully');
            } else {
                $this->session->set_flashdata('error_message', 'There is already a blog category with this name');
            }
            redirect(site_url('admin/blog_category'), 'refresh');
            ...
            $this->session->set_flashdata('flash_message', 'Blog category deleted successfully');
            redirect(site_url('admin/blog_category'), 'refresh');
        }
    ...
    public function edit_blog($blog_id = "")
    {
        ...
        $page_data['youngo_blog_english_translation'] = $this->crud_model->youngo_get_blog_translation($blog_id, 'english');
        $page_data['youngo_blog_arabic_translation'] = $this->crud_model->youngo_get_blog_translation($blog_id, 'arabic');
    ...
    public function blog($param1 = "", $param2 = "")
    {
        if ($param1 == 'add') {
            ...
            $this->session->set_flashdata('flash_message', 'Blog added successfully');
            redirect(site_url('admin/blog'), 'refresh');
        } elseif ($param1 == 'update') {
            $this->crud_model->update_blog($param2);
            $this->session->set_flashdata('flash_message', 'Blog updated successfully');
            redirect(site_url('admin/blog'), 'refresh');
        } elseif ($param1 == 'status') {
            $this->crud_model->update_blog_status($param2);
            $this->session->set_flashdata('flash_message', 'Blog status has been updated');
            redirect(site_url('admin/blog'), 'refresh');
        } elseif ($param1 == 'delete') {
            $this->crud_model->blog_delete($param2);
            $this->session->set_flashdata('flash_message', 'Blog deleted successfully');
            redirect(site_url('admin/blog'), 'refresh');
        }
    ...
    public function language_import()
    {
        ...
        if (empty($_FILES['language_files']['name']) || !is_array($_FILES['language_files']['name'])) {
            $this->session->set_flashdata('error_message', get_phrase('JSON_validation_failed') . '!');
            redirect(site_url('admin/manage_language'), 'refresh');
        }

        foreach ($_FILES['language_files']['name'] as $key => $language_file_name) {
            $language_name = $this->youngo_normalize_legacy_import_language_name($language_file_name);
            if ($this->youngo_is_blocked_legacy_language_import_target($language_name)) {
                $this->session->set_flashdata('error_message', 'Arabic UI language import is protected. Use the safe Arabic import preview workflow; arabic_translated is not a UI language target.');
                redirect(site_url('admin/manage_language'), 'refresh');
            }
        }
        ...
        foreach ($_FILES['language_files']['name'] as $key => $language) {
            $language_name = $this->youngo_normalize_legacy_import_language_name($_FILES['language_files']['name'][$key]);
    ...
    protected function youngo_normalize_legacy_import_language_name($language_file_name)
    {
        $base_name = pathinfo((string) $language_file_name, PATHINFO_FILENAME);

        return strtolower(preg_replace('/\s+/', '_', $base_name));
    }

    protected function youngo_is_blocked_legacy_language_import_target($language_name)
    {
        $language_name = strtolower(trim((string) $language_name));

        return $language_name === 'arabic' || $language_name === 'arabic_translated';
    }

    protected function youngo_is_blocked_ui_language_target($language_name)
    {
        return strtolower(trim((string) $language_name)) === 'arabic_translated';
    }
