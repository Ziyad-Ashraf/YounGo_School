...
class Payment extends CI_Controller
{
    ...
    private function youngo_gateway_payment_check($payment_method = "", $payment_context = "course")
    {
        $payment_method = trim((string) $payment_method);
        if ($payment_method === '' || !preg_match('/^[a-z0-9_]+$/i', $payment_method)) {
            return false;
        }

        $payment_gateway = $this->db->get_where('payment_gateways', ['identifier' => $payment_method])->row_array();
        if (empty($payment_gateway) || empty($payment_gateway['model_name']) || (int) $payment_gateway['status'] !== 1) {
            return false;
        }
        ...
        if ($system_currency !== '' && $gateway_currency !== $system_currency) {
            return false;
        }

        $model_name = strtolower(trim((string) $payment_gateway['model_name']));
        if ($model_name === '') {
            return false;
        }

        if ((int) $payment_gateway['is_addon'] === 1) {
            $this->load->model('addons/' . $model_name);
        }
        ...
        if (!method_exists($this->$model_name, $payment_check_function)) {
            return false;
        }

        return $this->$model_name->$payment_check_function($payment_method, $payment_context);
    }
    ...
    function success_course_payment($payment_method = "")
    {
        ...
        $response = $this->youngo_gateway_payment_check($payment_method, 'course');
        if ($response === true) {
            //if course is a gift purchase
            $gift_to_user_id = isset($payment_details['gift_to_user_id']) ? (int) $payment_details['gift_to_user_id'] : 0;
            $total_payable_amount = isset($payment_details['total_payable_amount']) ? $payment_details['total_payable_amount'] : 0;
            if ($gift_to_user_id > 0) {
                $enrol_user_id = $gift_to_user_id;
                $this->crud_model->enrol_student($enrol_user_id, $payer_user_id);
                $this->email_model->course_gift_notification($enrol_user_id, $payer_user_id, $payment_method, $total_payable_amount);
            } else {

                $this->crud_model->enrol_student($enrol_user_id);
                $this->email_model->course_purchase_notification($enrol_user_id, $payment_method, $total_payable_amount);
            }
            $this->crud_model->course_purchase($payer_user_id, $payment_method, $total_payable_amount);
    ...
    function success_instructor_payment($payment_method = "")
    {
        ...
        $response = $this->youngo_gateway_payment_check($payment_method, 'instructor');

        if ($response) {
            $payout_id = isset($payment_details['items'][0]['payout_id']) ? (int) $payment_details['items'][0]['payout_id'] : 0;
            if ($payout_id <= 0) {
                $this->session->set_flashdata('error_message', site_phrase('an_error_occurred_during_payment'));
                redirect(site_url('admin/instructor_payout'), 'refresh');
            }

            $this->crud_model->update_payout_status($payout_id, $payment_method);
