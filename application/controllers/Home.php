...
class Home extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (get_settings('course_accessibility') != 'publicly' && !$this->session->userdata('user_id')) {
            redirect($this->youngo_frontend_public_redirect_url('login'), 'refresh');
        }
    ...
    private function youngo_course_access_state_for_request($course_id, $options = array())
    {
        $course_id = (int) $course_id;
        $user_id = (int) $this->session->userdata('user_id');
        $allow_admin_bypass = array_key_exists('allow_admin_bypass', $options) ? (bool) $options['allow_admin_bypass'] : true;
        $allow_instructor_bypass = array_key_exists('allow_instructor_bypass', $options) ? (bool) $options['allow_instructor_bypass'] : true;
        ...
        $entitlement_helper_path = APPPATH . 'helpers/youngo_entitlement_helper.php';
        $entitlement_model_path = APPPATH . 'models/Youngo_entitlement_model.php';

        if (file_exists($entitlement_helper_path) && file_exists($entitlement_model_path)) {
            $this->load->helper('youngo_entitlement');
        }

        if (function_exists('youngo_get_course_access_state') && file_exists($entitlement_model_path)) {
            try {
                return youngo_get_course_access_state($user_id, $course_id, array_merge(array(
                    'allow_admin_bypass' => $allow_admin_bypass,
                    'allow_instructor_bypass' => $allow_instructor_bypass,
                ), $options));
            } catch (Throwable $exception) {
                // Fall through to legacy-compatible access fallback below.
            }
        }

        $course_details = $this->crud_model->get_course_by_id($course_id)->row_array();
        $is_course_instructor = false;
        if (!empty($course_details) && $allow_instructor_bypass) {
            $is_course_instructor = $this->crud_model->is_course_instructor($course_id, $user_id);
        }

        $enroll_status = enroll_status($course_id);
        $is_admin = $allow_admin_bypass && ((int) $this->session->userdata('admin_login') === 1 || (int) $this->session->userdata('role_id') === 1);
        $legacy_access_allowed = $is_admin || $is_course_instructor || $enroll_status == 'valid';

        $default_state['has_access'] = $legacy_access_allowed;
        $default_state['lesson_access_allowed'] = $legacy_access_allowed;
        $default_state['access_source'] = $is_admin ? 'admin' : ($is_course_instructor ? 'instructor' : ($enroll_status == 'valid' ? 'legacy_enrol' : 'none'));
        $default_state['status'] = $enroll_status == 'expired' ? 'expired' : ($legacy_access_allowed ? 'active' : 'none');
        $default_state['message_key'] = $legacy_access_allowed ? 'access_active' : ($enroll_status == 'expired' ? 'access_expired' : 'access_none');

        return $default_state;
    }

    private function youngo_course_access_ajax_error($access_state)
    {
        if (isset($access_state['status']) && $access_state['status'] === 'expired') {
            return get_phrase('Your course access has expired');
        }

        if (isset($access_state['status']) && $access_state['status'] === 'revoked') {
            return get_phrase('Your course access is not active');
        }

        if (!function_exists('youngo_frontend_phrase') && file_exists(APPPATH . 'helpers/youngo_frontend_language_helper.php')) {
            $this->load->helper('youngo_frontend_language');
        }

        return function_exists('youngo_frontend_phrase')
            ? youngo_frontend_phrase('subscribe_to_unlock_this_course')
            : get_phrase('Subscribe to a YounGo plan to unlock this course and all available courses.');
    }

    private function youngo_course_uses_managed_access($course_details)
    {
        if (empty($course_details) || !is_array($course_details)) {
            return false;
        }

        $mode = isset($course_details['youngo_access_mode']) ? trim((string) $course_details['youngo_access_mode']) : '';
        return in_array($mode, array('subscription_only', 'subscription_and_purchase', 'purchase_only'), true);
    }

    private function youngo_managed_access_message($course_details)
    {
        $mode = isset($course_details['youngo_access_mode']) ? trim((string) $course_details['youngo_access_mode']) : '';
        if ($mode === 'subscription_only' && empty($course_details['is_free_course'])) {
            return get_phrase('Subscription checkout is not available yet');
        }

        return get_phrase('Access for this course is managed by your school/admin.');
    }

    private function youngo_course_is_legacy_cart_allowed($course_id)
    {
        $course_id = (int) $course_id;
        if ($course_id <= 0) {
            return false;
        ...
        return !$this->youngo_course_uses_managed_access($course_details);
    }

    private function youngo_remove_managed_access_courses_from_cart()
    {
        ...
        foreach ($cart_items as $cart_item) {
            $course_id = (int) $cart_item;
            if ($course_id > 0 && $this->youngo_course_is_legacy_cart_allowed($course_id)) {
                $kept_items[] = $cart_item;
                continue;
            }

            $removed_items[] = $cart_item;
        }

        if (!empty($removed_items)) {
            $this->session->set_userdata('cart_items', $kept_items);
        }

        return $removed_items;
    }

    private function youngo_frontend_content_language_for_request()
    {
        if (file_exists(APPPATH . 'helpers/youngo_frontend_content_helper.php')) {
            $this->load->helper('youngo_frontend_content');
            if (function_exists('youngo_frontend_content_language')) {
                return youngo_frontend_content_language();
            }
        }
    ...
    private function youngo_frontend_language_for_public_redirect()
    {
        if (file_exists(APPPATH . 'helpers/youngo_frontend_language_helper.php')) {
            $this->load->helper('youngo_frontend_language');
            if (function_exists('youngo_frontend_active_language')) {
                return youngo_frontend_active_language($this->uri->uri_string());
            }
        }
    ...
    private function youngo_frontend_public_redirect_url($uri_string = '')
    {
        $language = $this->youngo_frontend_language_for_public_redirect();
        if (file_exists(APPPATH . 'helpers/youngo_frontend_language_helper.php')) {
            $this->load->helper('youngo_frontend_language');
            if (function_exists('youngo_frontend_public_url')) {
                return youngo_frontend_public_url($uri_string, $language);
            }
        }
    ...
    private function youngo_frontend_public_course_redirect_url($course_details)
    {
        $language = $this->youngo_frontend_language_for_public_redirect();
        if (file_exists(APPPATH . 'helpers/youngo_frontend_content_helper.php')) {
            $this->load->helper('youngo_frontend_content');
            if (function_exists('youngo_frontend_course_detail_url')) {
                return youngo_frontend_course_detail_url($course_details, $language);
            }
        }

        return site_url('home/course/' . slugify($course_details['title']) . '/' . (int) $course_details['id']);
    }

    private function youngo_redirect_to_frontend_public($uri_string = '')
    {
        redirect($this->youngo_frontend_public_redirect_url($uri_string), 'refresh');
    }
    ...
    private function youngo_frontend_public_arabic_safe_phrase($phrase_key, $fallback = '')
    {
        $value = $this->youngo_frontend_public_phrase($phrase_key, $fallback);
        $language = $this->youngo_frontend_language_for_public_redirect();

        if ($language !== 'arabic' || preg_match('/\p{Arabic}/u', (string) $value)) {
            return $value;
        }
    ...
    private function youngo_frontend_pagination_offset_from_uri()
    {
        ...
        return ctype_digit((string) $last_segment) ? (int) $last_segment : 0;
    }
    ...
    {
        $page_data['youngo_frontend_language'] = $this->youngo_frontend_content_language_for_request();
        ...
        $page_data['page_title'] = $this->youngo_frontend_public_phrase('home');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }

    public function subscriptions()
    {
        $youngo_frontend_language = $this->youngo_frontend_content_language_for_request();
        ...
        $page_data['subscription_plans'] = $this->youngo_subscription_model->get_public_subscription_plans($youngo_frontend_language);
        $page_data['page_name'] = 'subscriptions';
        $page_data['page_title'] = youngo_frontend_phrase('subscriptions', $youngo_frontend_language);
    ...
    public function courses()
    {
        ...
        if ($search_string == "" && $selected_category_id == "all" && $selected_price == "all" && $selected_level == 'all' && $selected_language == 'all' && $selected_rating == 'all' && $selected_sorting == 'newest') {
            ...
            $config['base_url']  = function_exists('youngo_frontend_courses_url') ? youngo_frontend_courses_url($youngo_frontend_language) . '/' : site_url('home/courses/');
            ...
            $page_data['courses'] = $this->db->get('course', $config['per_page'], $this->youngo_frontend_pagination_offset_from_uri())->result_array();
            if (function_exists('youngo_frontend_translate_course_rows')) {
                $page_data['courses'] = youngo_frontend_translate_course_rows($page_data['courses'], $youngo_frontend_language);
            }
            ...
            $config['first_url']  = function_exists('youngo_frontend_courses_url') ? youngo_frontend_courses_url($youngo_frontend_language, $search_string_val . 'category=' . $category_slug . '&price=' . $selected_price . '&level=' . $selected_level . '&language=' . $selected_language . '&rating=' . $selected_rating . '&sort_by=' . $selected_sorting) : site_url('home/courses') . '?' . $search_string_val . 'category=' . $category_slug . '&price=' . $selected_price . '&level=' . $selected_level . '&language=' . $selected_language . '&rating=' . $selected_rating . '&sort_by=' . $selected_sorting;

            $this->pagination->initialize($config);
            $courses = $this->crud_model->filter_course($search_string, $selected_category_id, $selected_price, $selected_level, $selected_language, $selected_rating, $selected_sorting, $config['per_page'], $this->youngo_frontend_pagination_offset_from_uri())->result_array();
        ...
        $page_data['page_title'] = $this->youngo_frontend_public_phrase('courses');
    ...
    public function course($slug = "", $course_id = "")
    {
        ...
        $page_data['page_title'] = $this->youngo_frontend_public_phrase('course');
    ...
    public function instructor_page($instructor_id = "")
    {
        ...
        $page_data['youngo_frontend_language'] = $this->youngo_frontend_content_language_for_request();
        $page_data['page_title'] = $this->youngo_frontend_public_phrase(
            'instructor_profile',
            $page_data['youngo_frontend_language'] === 'arabic' ? 'ملف المدرّس' : 'Instructor profile'
        );
    ...
    public function my_courses()
    {
        if ($this->session->userdata('user_login') != true) {
            $this->youngo_redirect_to_frontend_public('');
        }

        $user_id = (int) $this->session->userdata('user_id');
        $this->load->model('Youngo_entitlement_model', 'youngo_entitlement_model');
        $youngo_frontend_language = $this->youngo_frontend_content_language_for_request();
        $learner_course_access_items = $this->youngo_entitlement_model->get_learner_course_access_items($user_id);
        if (function_exists('youngo_frontend_translate_learner_course_items')) {
            $learner_course_access_items = youngo_frontend_translate_learner_course_items($learner_course_access_items, $youngo_frontend_language);
        }

        $page_data['page_name'] = "my_courses";
        $page_data['page_title'] = $this->youngo_frontend_public_phrase("my_courses");
        $page_data['youngo_frontend_language'] = $youngo_frontend_language;
        $page_data['learner_course_access_items'] = $learner_course_access_items;
        $page_data['learner_subscription_summary'] = $this->youngo_entitlement_model->get_learner_subscription_summary($user_id);
        $page_data['learner_access_counts'] = $this->youngo_entitlement_model->get_learner_access_counts($user_id);
    ...
    public function my_access()
    {
        ...
        $page_data['page_name'] = "my_access";
        $page_data['page_title'] = $this->youngo_frontend_public_phrase("My Access");
        $page_data['youngo_frontend_language'] = $youngo_frontend_language;
        $page_data['learner_course_access_items'] = $learner_course_access_items;
        $page_data['learner_subscription_summary'] = $this->youngo_entitlement_model->get_learner_subscription_summary($user_id);
        $page_data['learner_access_counts'] = $this->youngo_entitlement_model->get_learner_access_counts($user_id);
    ...
    public function my_wishlist()
    {
        ...
        if (function_exists('youngo_frontend_translate_course_rows')) {
            $my_courses = youngo_frontend_translate_course_rows($my_courses, $youngo_frontend_language);
        }
        $page_data['youngo_frontend_language'] = $youngo_frontend_language;
        $page_data['my_courses'] = $my_courses;
        $page_data['page_name'] = "my_wishlist";
        $page_data['page_title'] = $this->youngo_frontend_public_arabic_safe_phrase('my_wishlist');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    function toggleWishlistItems($course_id = "", $identifier = "")
    {
        if ($this->session->userdata('user_login') != 1) {
            ...
            if ($response_bool) {
                $response['success'] = $this->youngo_frontend_public_phrase('course_added_to_wishlist', 'Course added to wishlist');
            } else {
                $response['success'] = $this->youngo_frontend_public_phrase('course_removed_from_wishlist', 'Course removed from wishlist');
            }
    ...
    public function handleCartItems($return_number = "")
    {
        ...
        if (!in_array($course_id, $previous_cart_items) && !$this->youngo_course_is_legacy_cart_allowed($course_id)) {
            if ($return_number == 'true') {
    ...
    public function handle_cart_items($course_id = "", $identifier = "")
    {
        ...
        if (!in_array($course_id, $previous_cart_items) && !$this->youngo_course_is_legacy_cart_allowed($course_id)) {
            $course_details = $this->crud_model->get_course_by_id($course_id)->row_array();
            $response['error'] = $this->youngo_managed_access_message($course_details);
            $response['html'] = [
                'elem' => '#shoppingCart',
            ...
            $response['text'] = ['elem' => '#cartItemsCounter', 'content' => count($previous_cart_items)];
            echo json_encode($response);
            return;
        }
    ...
    public function handle_buy_now($course_id = "")
    {
        ...
        if (!$this->youngo_course_is_legacy_cart_allowed($course_id)) {
            $course_details = $this->crud_model->get_course_by_id($course_id)->row_array();
            $redirect_url = !empty($course_details)
                ? site_url('home/course/' . slugify($course_details['title']) . '/' . (int) $course_id)
                : site_url('home/courses');
            echo json_encode([
                'error' => $this->youngo_managed_access_message($course_details),
    ...
    public function lesson($slug = "", $course_id = "", $lesson_id = "")
    {
        ...
        if (function_exists('youngo_get_course_access_state') && file_exists($entitlement_model_path)) {
            $access_state = youngo_get_course_access_state($user_id, $course_id, array(
                'allow_admin_bypass' => true,
                'allow_instructor_bypass' => true,
                'gate' => 'lesson',
                'require_lesson_course_match' => true,
            ));
        } else {
            $enroll_status = enroll_status($course_id);
            $legacy_access_allowed = $this->session->userdata('role_id') == 1 || in_array($user_id, $course_instructor_ids) || $enroll_status == 'valid';
            $access_state = array(
                'lesson_access_allowed' => $legacy_access_allowed,
                'status' => $enroll_status == 'expired' ? 'expired' : ($legacy_access_allowed ? 'active' : 'none'),
            );
        }

        if (empty($access_state['lesson_access_allowed'])) {
            if (isset($access_state['status']) && $access_state['status'] === 'expired') {
                $this->session->set_flashdata('error_message', site_phrase('Your course access has expired'));
            } else {
                $this->session->set_flashdata('error_message', site_phrase('Access denied'));
            }
            redirect($this->youngo_frontend_public_course_redirect_url($course_details), 'refresh');
        }

        if ($course_details['course_type'] == 'general' && $lesson_id != "") {
            $requested_lesson_details = $this->crud_model->get_lessons('lesson', $lesson_id)->row_array();
            if (empty($requested_lesson_details) || $requested_lesson_details['course_id'] != $course_id) {
                $this->session->set_flashdata('error_message', site_phrase('Access denied'));
                $this->youngo_redirect_to_frontend_public('');
            }
        }
        ...
        if ($course_details['course_type'] == 'general') {
            ...
            if ($sections->num_rows() > 0) {
                ...
                if ($lesson_id == "") {
                    ...
                    $section_id = $lesson_row->section_id;
        ...
        if ($course_details['course_type'] == 'general' && (empty($lesson_details) || $lesson_details['course_id'] != $course_id)) {
            $this->session->set_flashdata('error_message', site_phrase('Access denied'));
            $this->youngo_redirect_to_frontend_public('');
        }
        ...
        $lesson_uri = trim($this->uri->uri_string(), '/');
        $youngo_frontend_language = (strpos($lesson_uri, 'en/') === 0 || $lesson_uri === 'en')
            ? 'english'
            : 'arabic';
        ...
        if (is_array($lesson_details) && function_exists('youngo_frontend_translate_lesson_row')) {
            $lesson_details = youngo_frontend_translate_lesson_row($lesson_details, $youngo_frontend_language);
        }
        if (isset($page_data['sections']) && function_exists('youngo_frontend_translate_section_rows')) {
            $page_data['sections'] = youngo_frontend_translate_section_rows($page_data['sections'], $youngo_frontend_language);
        }
    ...
    {
        $course_id = (int) $course_id;
        $access_state = $this->youngo_course_access_state_for_request($course_id, array(
            'allow_admin_bypass' => true,
            'allow_instructor_bypass' => true,
            'gate' => 'pdf_canvas',
        ));

        if (!empty($access_state['lesson_access_allowed']) || get_bundle_validity($bundle_id) == 'valid') {
            $page_data['course_id'] = $course_id;
            $page_data['lesson_id'] = $lesson_id;
    ...
    public function my_courses_by_category()
    {
        ...
        $user_id = (int) $this->session->userdata('user_id');
        $this->load->model('Youngo_entitlement_model', 'youngo_entitlement_model');
        $items = $this->youngo_entitlement_model->get_learner_course_access_items($user_id);

        if ($category_id > 0) {
            $items = array_values(array_filter($items, function ($item) use ($category_id) {
                $course = isset($item['course']) && is_array($item['course']) ? $item['course'] : array();
                return (isset($course['category_id']) && (int) $course['category_id'] === $category_id)
                    || (isset($course['sub_category_id']) && (int) $course['sub_category_id'] === $category_id);
            }));
        }

        $page_data['learner_course_access_items'] = $items;
    ...
    public function search($search_string = "")
    {
        ...
        if (isset($_GET['query']) && !empty($_GET['query'])) {
            ...
            $config['base_url']  = function_exists('youngo_frontend_search_url') ? youngo_frontend_search_url($youngo_frontend_language) . '/' : site_url('home/search/');
            $config['suffix']  = '?query=' . $search_string;
            $config['first_url']  = function_exists('youngo_frontend_search_url') ? youngo_frontend_search_url($youngo_frontend_language, 'query=' . $search_string) : site_url('home/search') . '?query=' . $search_string;
            $this->pagination->initialize($config);

            $page_data['courses'] = $this->crud_model->get_courses_by_search_string($search_string, $config['per_page'], $this->youngo_frontend_pagination_offset_from_uri())->result_array();
            if (function_exists('youngo_frontend_translate_course_rows')) {
                $page_data['courses'] = youngo_frontend_translate_course_rows($page_data['courses'], $youngo_frontend_language);
            }
    ...
    public function my_courses_by_search_string()
    {
        ...
        $search_string = trim((string) $this->input->post('search_string'));
        $user_id = (int) $this->session->userdata('user_id');
        $this->load->model('Youngo_entitlement_model', 'youngo_entitlement_model');
        $items = $this->youngo_entitlement_model->get_learner_course_access_items($user_id);

        if ($search_string !== '') {
            $items = array_values(array_filter($items, function ($item) use ($search_string) {
                $title = isset($item['title']) ? $item['title'] : '';
                return stripos($title, $search_string) !== false;
            }));
        }
    ...
    public function rate_course()
    {
        ...
        if (!empty($access_state['has_access'])) {
            ...
            $response['error'] = get_phrase('This course can be rated only with active course access');
        }
    ...
    public function get_enrolled_to_free_course($course_id)
    {
        ...
        if (empty($course_details)) {
            $this->session->set_flashdata('error_message', site_phrase('Access denied'));
            $this->youngo_redirect_to_frontend_public('home/courses');
        }

        if ($this->youngo_course_uses_managed_access($course_details)) {
            $this->session->set_flashdata('error_message', $this->youngo_managed_access_message($course_details));
            redirect($this->youngo_frontend_public_course_redirect_url($course_details), 'refresh');
        }
        ...
        if ($this->session->userdata('user_login') == 1) {
            ...
            $url = $this->youngo_frontend_public_course_redirect_url($course_details);
            set_url_history($url);
            redirect($this->youngo_frontend_public_redirect_url('login'), 'refresh');
        }
    }
    ...
    public function login()
    {
        ...
        $page_data['page_title'] = $this->youngo_frontend_public_phrase('login');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function sign_up()
    {
        ...
        $page_data['page_title'] = $this->youngo_frontend_public_phrase('sign_up');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function forgot_password()
    {
        ...
        $page_data['page_title'] = $this->youngo_frontend_public_phrase('forgot_password');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    function play_lesson($lesson_id = "", $is_preview = "")
    {
        ...
        if (empty($lesson)) {
            $response['error'] = get_phrase('Access denied');
            echo json_encode($response);
            return;
        }
        ...
        if ($is_preview) {
            ...
            $access_state = $this->youngo_course_access_state_for_request($course_details['id'], array(
                'allow_admin_bypass' => true,
                'allow_instructor_bypass' => true,
                'gate' => 'ajax_lesson',
            ));

            if (!empty($access_state['lesson_access_allowed']) || $admin_login || $is_course_instructor) {
                $response['redirectTo'] = site_url('home/lesson/' . slugify($course_details['title']) . '/' . $course_details['id'] . '/' . $lesson['id']);
                echo json_encode($response);
                return;
            }

            $response['error'] = $this->youngo_course_access_ajax_error($access_state);
            echo json_encode($response);
        }
    }
    ...
    function course_payment()
    {
        ...
        if (!empty($this->youngo_remove_managed_access_courses_from_cart())) {
            $this->session->set_flashdata('error_message', get_phrase('Subscription checkout is not available yet'));
            redirect(site_url('home/shopping_cart'), 'refresh');
        }
    ...
    function contact_us($param1 = "")
    {
        ...
        $page_data['page_title'] = $this->youngo_frontend_public_phrase('Contact us');
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    function coupon_offer_100_percent()
    {
        ...
        if (!empty($coupon_details) && count($cart_items) > 0 && $coupon_code && $coupon_details['discount_percentage'] == 100 && $coupon_details['expiry_date'] >= time() && $enrol_user_id > 0 && $this->session->userdata('user_login')) {

            $this->session->set_flashdata('error_message', site_phrase('Coupon-based checkout is not available yet'));
            redirect('home/shopping_cart', 'refresh');
        } else {
    ...
    function lesson_mobile_web_view_get($lesson_id = "")
    {
        ...
        $access_state = $this->youngo_course_access_state_for_request($lesson_details['course_id'], array(
            'allow_admin_bypass' => true,
            'allow_instructor_bypass' => true,
            'gate' => 'mobile_web_lesson',
        ));
    ...
    function offline_video_for_mobile_app($lesson_id = "")
    {
        ...
        if ($lesson_video->num_rows() == 0) {
            return;
        }

        $video_url = $lesson_video->row('video_url');
        $access_state = $this->youngo_course_access_state_for_request($lesson_video->row('course_id'), array(
            'allow_admin_bypass' => true,
            'allow_instructor_bypass' => true,
            'gate' => 'offline_video',
        ));

        if (!empty($access_state['lesson_access_allowed']) && $video_url != '') {
