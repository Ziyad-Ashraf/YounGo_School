...
class Files extends CI_Controller
{
    ...
    function index()
    {
        ...
        if (isset($_GET['course_id']) && isset($_GET['lesson_id']) && $user_id > 0) {
            ...
            $lesson_query = $this->crud_model->get_lessons('lesson', $lesson_id);
            if ($lesson_query->num_rows() <= 0) {
                return false;
            }

            $lesson = $lesson_query->row_array();
            if ((int) $lesson['course_id'] !== (int) $course_id) {
                return false;
            }
            ...
            if (function_exists('youngo_get_course_access_state') && file_exists($entitlement_model_path)) {
                ...
                $instructor_access_allowed = in_array($user_id, $multi_instructors) || (isset($course_details['creator']) && $course_details['creator'] == $user_id);
                $legacy_access_allowed = $this->session->userdata('admin_login') == '1' || $this->session->userdata('role_id') == 1 || $instructor_access_allowed || $enroll_status == 'valid';
                $access_state = array(
