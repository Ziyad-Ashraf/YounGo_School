...
class Crud_model extends CI_Model
{
    ...
    protected function get_posted_category_translation_fields()
    {
        $english_name = trim((string) $this->input->post('english_name'));
        if ($english_name === '') {
            $english_name = trim((string) $this->input->post('name'));
        }

        $english_slug = trim((string) $this->input->post('english_slug'));
        $arabic_name  = trim((string) $this->input->post('arabic_name'));
        $arabic_slug  = trim((string) $this->input->post('arabic_slug'));

        $this->load->model('youngo_translation_model');

        if ($english_slug === '' && $english_name !== '') {
            $english_slug = $this->youngo_translation_model->generate_slug($english_name, 'english');
        }

        if ($arabic_slug === '' && $arabic_name !== '') {
            $arabic_slug = $this->youngo_translation_model->generate_slug($arabic_name, 'arabic');
        }
    ...
    protected function get_posted_category_parent()
    {
        $parent = $this->input->post('parent');
        if ($parent === null || $parent === '') {
            $parent = $this->input->post('category_id');
        }

        return (int) $parent;
    }

    protected function sync_category_translations($category_id, array $translation_fields)
    {
        ...
        $english_result = $this->youngo_translation_model->upsert_category_translation($category_id, 'english', array(
            'name' => $translation_fields['english_name'],
            'slug' => $translation_fields['english_slug'],
        ));

        if (empty($english_result['success'])) {
            log_message('error', 'Phase 2U.5 category English translation sync failed for category_id ' . $category_id . ': ' . (isset($english_result['code']) ? $english_result['code'] : 'unknown'));
            return false;
        }

        if ($translation_fields['arabic_name'] !== '' || $translation_fields['arabic_slug'] !== '') {
            $arabic_result = $this->youngo_translation_model->upsert_category_translation($category_id, 'arabic', array(
                'name' => $translation_fields['arabic_name'],
                'slug' => $translation_fields['arabic_slug'],
            ));

            if (empty($arabic_result['success'])) {
                log_message('error', 'Phase 2U.5 category Arabic translation sync failed for category_id ' . $category_id . ': ' . (isset($arabic_result['code']) ? $arabic_result['code'] : 'unknown'));
                return false;
            }
        }
    ...
    protected function get_posted_course_translation_fields()
    {
        $this->load->model('youngo_translation_model');

        $english_title = trim((string) $this->input->post('english_title'));
        if ($english_title === '') {
            $english_title = trim((string) $this->input->post('title'));
        }

        $english_slug = trim((string) $this->input->post('english_slug'));
        if ($english_slug === '' && $english_title !== '') {
            $english_slug = $this->youngo_translation_model->generate_slug($english_title, 'english');
        }

        $arabic_title = trim((string) $this->input->post('arabic_title'));
        $arabic_slug = trim((string) $this->input->post('arabic_slug'));
        if ($arabic_slug === '' && $arabic_title !== '') {
            $arabic_slug = $this->youngo_translation_model->generate_slug($arabic_title, 'arabic');
        }
        ...
        return array(
            'english' => array(
                'title' => $english_title,
                'slug' => $english_slug,
                'short_description' => (string) $this->youngo_post_value_with_fallback('english_short_description', 'short_description'),
                'description' => remove_js($this->youngo_post_value_with_fallback('english_description', 'description', false)),
                'outcomes' => $this->youngo_post_array_json_with_fallback('english_outcomes', 'outcomes'),
                'requirements' => $this->youngo_post_array_json_with_fallback('english_requirements', 'requirements'),
                'faqs' => $this->youngo_post_faq_json_with_fallback('english_faqs', 'english_faq_descriptions', 'faqs', 'faq_descriptions'),
                'seo_title' => (string) $this->input->post('english_seo_title'),
                'meta_keywords' => (string) $this->youngo_post_value_with_fallback('english_meta_keywords', 'meta_keywords'),
                'meta_description' => (string) $this->youngo_post_value_with_fallback('english_meta_description', 'meta_description'),
            ),
            'arabic' => array(
                'title' => $arabic_title,
                'slug' => $arabic_slug,
                'short_description' => (string) $this->input->post('arabic_short_description'),
                'description' => remove_js($this->input->post('arabic_description', false)),
                'outcomes' => $this->youngo_post_array_json('arabic_outcomes'),
                'requirements' => $this->youngo_post_array_json('arabic_requirements'),
                'faqs' => $this->youngo_post_faq_json('arabic_faqs', 'arabic_faq_descriptions'),
                'seo_title' => (string) $this->input->post('arabic_seo_title'),
                'meta_keywords' => (string) $this->input->post('arabic_meta_keywords'),
                'meta_description' => (string) $this->input->post('arabic_meta_description'),
            ),
        );
    }

    protected function youngo_post_value_with_fallback($primary_field, $fallback_field, $xss_clean = true)
    {
        $primary_value = $this->input->post($primary_field, $xss_clean);
        if ($primary_value !== null && $primary_value !== '') {
            return $primary_value;
        }

        return $this->input->post($fallback_field, $xss_clean);
    }

    protected function youngo_post_array_json($field)
    {
        $value = $this->input->post($field);
        if (is_array($value)) {
            return $this->trim_and_return_json($value);
        }

        if (is_scalar($value) && trim((string) $value) !== '') {
            return json_encode(array(trim((string) $value)));
        }

        return json_encode(array());
    }

    protected function youngo_post_array_json_with_fallback($primary_field, $fallback_field)
    {
        $primary_value = $this->input->post($primary_field);
        if (is_array($primary_value) && count(array_filter($primary_value, 'strlen')) > 0) {
            return $this->trim_and_return_json($primary_value);
        }

        if (is_scalar($primary_value) && trim((string) $primary_value) !== '') {
            return json_encode(array(trim((string) $primary_value)));
        }

        return $this->trim_and_return_json($this->input->post($fallback_field));
    }

    protected function youngo_post_faq_json($question_field, $answer_field)
    {
        $faqs = array();
        $questions = $this->input->post($question_field);
        $answers = $this->input->post($answer_field);
        ...
        foreach (array_filter($questions) as $faq_key => $faq_title) {
            $faq_title = trim((string) $faq_title);
            if ($faq_title === '') {
                continue;
            }

            $faqs[$faq_title] = isset($answers[$faq_key]) ? (string) $answers[$faq_key] : '';
        }

        return json_encode($faqs);
    }

    protected function youngo_post_faq_json_with_fallback($primary_question_field, $primary_answer_field, $fallback_question_field, $fallback_answer_field)
    {
        $primary_questions = $this->input->post($primary_question_field);
        if (is_array($primary_questions) && count(array_filter($primary_questions, 'strlen')) > 0) {
            return $this->youngo_post_faq_json($primary_question_field, $primary_answer_field);
        }

        return $this->youngo_post_faq_json($fallback_question_field, $fallback_answer_field);
    }

    protected function youngo_course_translation_has_content(array $translation_fields)
    {
        foreach ($translation_fields as $value) {
            if (is_scalar($value) && trim((string) $value) !== '' && trim((string) $value) !== '[]' && trim((string) $value) !== '{}') {
                return true;
            }
        }
    ...
    protected function sync_course_translations($course_id, array $translation_fields)
    {
        $course_id = (int) $course_id;
        if ($course_id <= 0 || empty($translation_fields['english']) || !is_array($translation_fields['english'])) {
            return false;
        }

        $this->load->model('youngo_translation_model');

        $english_result = $this->youngo_translation_model->upsert_course_translation($course_id, 'english', $translation_fields['english']);
        if (empty($english_result['success'])) {
            log_message('error', 'Phase 2U.5 course English translation sync failed for course_id ' . $course_id . ': ' . (isset($english_result['code']) ? $english_result['code'] : 'unknown'));
            return false;
        }

        if (!empty($translation_fields['arabic']) && is_array($translation_fields['arabic']) && $this->youngo_course_translation_has_content($translation_fields['arabic'])) {
            $arabic_result = $this->youngo_translation_model->upsert_course_translation($course_id, 'arabic', $translation_fields['arabic']);
            if (empty($arabic_result['success'])) {
                log_message('error', 'Phase 2U.5 course Arabic translation sync failed for course_id ' . $course_id . ': ' . (isset($arabic_result['code']) ? $arabic_result['code'] : 'unknown'));
                return false;
            }
        }
    ...
    protected function get_posted_section_translation_fields()
    {
        $english_title = trim((string) $this->input->post('english_title'));
        if ($english_title === '') {
            $english_title = trim((string) $this->input->post('title'));
    ...
    protected function sync_section_translations($section_id, array $translation_fields)
    {
        ...
        $english_result = $this->youngo_translation_model->upsert_section_translation($section_id, 'english', array(
            'title' => $translation_fields['english_title'],
        ));

        if (empty($english_result['success'])) {
            log_message('error', 'Phase 2U.5 section English translation sync failed for section_id ' . $section_id . ': ' . (isset($english_result['code']) ? $english_result['code'] : 'unknown'));
            return false;
        }

        if ($translation_fields['arabic_title'] !== '') {
            $arabic_result = $this->youngo_translation_model->upsert_section_translation($section_id, 'arabic', array(
                'title' => $translation_fields['arabic_title'],
            ));

            if (empty($arabic_result['success'])) {
                log_message('error', 'Phase 2U.5 section Arabic translation sync failed for section_id ' . $section_id . ': ' . (isset($arabic_result['code']) ? $arabic_result['code'] : 'unknown'));
                return false;
            }
        }
    ...
    protected function get_posted_lesson_translation_fields($lesson_type = '', $attachment_type = '')
    {
        $english_title = trim((string) $this->input->post('english_title'));
        if ($english_title === '') {
            $english_title = trim((string) $this->input->post('title'));
        ...
        $is_text_lesson = $lesson_type === 'text' && $attachment_type === 'description';

        $english_text_content = '';
        $arabic_text_content = '';
        if ($is_text_lesson) {
            $english_text_content = remove_js((string) $this->youngo_post_value_with_fallback('english_text_content', 'text_description', false));
            $arabic_text_content = remove_js((string) $this->input->post('arabic_text_content', false));
        }

        return array(
            'english' => array(
                'title' => $english_title,
                'summary' => remove_js((string) $this->youngo_post_value_with_fallback('english_summary', 'summary', false)),
                'text_content' => $english_text_content,
            ),
            'arabic' => array(
                'title' => trim((string) $this->input->post('arabic_title')),
                'summary' => remove_js((string) $this->input->post('arabic_summary', false)),
                'text_content' => $arabic_text_content,
            ),
        );
    }

    protected function youngo_lesson_translation_has_content(array $translation_fields)
    {
        foreach ($translation_fields as $value) {
            if (is_scalar($value) && trim((string) $value) !== '') {
                return true;
    ...
    protected function sync_legacy_lesson_post_fields(array $translation_fields)
    {
        if (empty($translation_fields['english']) || !is_array($translation_fields['english'])) {
            return;
        }

        $_POST['title'] = isset($translation_fields['english']['title']) ? $translation_fields['english']['title'] : '';
        $_POST['summary'] = isset($translation_fields['english']['summary']) ? $translation_fields['english']['summary'] : '';

        if (array_key_exists('text_content', $translation_fields['english'])) {
            $_POST['text_description'] = $translation_fields['english']['text_content'];
        }
    }

    protected function sync_lesson_translations($lesson_id, array $translation_fields)
    {
        $lesson_id = (int) $lesson_id;
        if ($lesson_id <= 0 || empty($translation_fields['english']) || !is_array($translation_fields['english'])) {
            return false;
        }

        $this->load->model('youngo_translation_model');

        $english_result = $this->youngo_translation_model->upsert_lesson_translation($lesson_id, 'english', $translation_fields['english']);
        if (empty($english_result['success'])) {
            log_message('error', 'Phase 2U.5 lesson English translation sync failed for lesson_id ' . $lesson_id . ': ' . (isset($english_result['code']) ? $english_result['code'] : 'unknown'));
            return false;
        }

        if (!empty($translation_fields['arabic']) && is_array($translation_fields['arabic']) && $this->youngo_lesson_translation_has_content($translation_fields['arabic'])) {
            $arabic_result = $this->youngo_translation_model->upsert_lesson_translation($lesson_id, 'arabic', $translation_fields['arabic']);
            if (empty($arabic_result['success'])) {
                log_message('error', 'Phase 2U.5 lesson Arabic translation sync failed for lesson_id ' . $lesson_id . ': ' . (isset($arabic_result['code']) ? $arabic_result['code'] : 'unknown'));
                return false;
            }
        }
    ...
    {
        $translation_fields = $this->get_posted_category_translation_fields();

        $data['code']   = html_escape($this->input->post('code'));
        $data['name']   = html_escape($translation_fields['english_name']);
        $data['parent'] = $this->get_posted_category_parent();
        $data['slug']   = html_escape($translation_fields['english_slug']);
        ...
        if ($previous_data == 0) {
            ...
            if ($data['parent'] == 0) {
                ...
                if (!isset($_FILES['category_thumbnail']['name']) || $_FILES['category_thumbnail']['name'] == "") {
             ...
             if ($data['parent']) {
                ...
                if (!isset($_FILES['sub_category_thumbnail']['name']) || $_FILES['sub_category_thumbnail']['name'] == "") {
            ...
            return $this->sync_category_translations($category_id, $translation_fields);
        }
    ...
    public function edit_category($param1)
    {
        ...
        if ($checker) {
            ...
            return $this->sync_category_translations($param1, $translation_fields);
        }
        return false;
    }
   ...
   // Category Sub Category Image Delete
    ...
    public function add_course($param1 = "", $options = array())
    {
        if (!is_array($options)) {
            $options = array();
        }

        $course_translation_fields = $this->get_posted_course_translation_fields();
        $youngo_course_access_data = null;
        $youngo_access_settings_submitted = $this->youngo_post_value_is_one('youngo_access_settings_submitted');

        if (!empty($options['can_manage_youngo_course_access']) && $youngo_access_settings_submitted) {
            $youngo_course_access_validation = $this->normalize_youngo_course_access_settings_from_post();
            if (empty($youngo_course_access_validation['success'])) {
                $this->session->set_flashdata('error_message', $youngo_course_access_validation['message']);
                return false;
            }

            $youngo_course_access_data = $youngo_course_access_validation['data'];
        }
        ...
        $data['title'] = html_escape($course_translation_fields['english']['title']);
        $data['short_description'] = html_escape($course_translation_fields['english']['short_description']);
        $data['description'] = $course_translation_fields['english']['description'];
        $data['outcomes'] = $course_translation_fields['english']['outcomes'];
        $data['faqs'] = $course_translation_fields['english']['faqs'];
        ...
        $data['requirements'] = $course_translation_fields['english']['requirements'];
        $pricing_fields_posted = $this->input->post('price') !== null || $this->input->post('discounted_price') !== null;
        if ($pricing_fields_posted) {
            $data['price'] = $this->input->post('price');
            $data['discount_flag'] = $this->input->post('discount_flag') ? $this->input->post('discount_flag') : 0;
        ...
        $data['meta_description'] = $course_translation_fields['english']['meta_description'];
        $data['meta_keywords'] = $course_translation_fields['english']['meta_keywords'];
        ...
        if (is_array($youngo_course_access_data)) {
            foreach ($youngo_course_access_data as $field => $value) {
                $data[$field] = $value;
            }
        }
        ...
        if (!$this->sync_course_translations($course_id, $course_translation_fields)) {
            $this->session->set_flashdata('error_message', 'Course was saved, but bilingual course translation data could not be fully synchronized.');
        }
    ...
    function add_shortcut_course($param1 = "")
    {
        ...
        if ($data['is_free_course'] == 1 || $data['is_free_course'] != 1 && $data['price'] > 0 && $data['discount_flag'] != 1 || $data['discount_flag'] == 1 && $data['discounted_price'] > 0) {
            ...
            $this->sync_course_translations($course_id, $course_translation_fields);
    ...
    private function normalize_youngo_course_access_settings_from_post()
    {
        if (!$this->youngo_post_value_is_scalar('youngo_access_mode')) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Please choose a valid access mode.',
            );
        }

        $access_mode = trim((string) $this->input->post('youngo_access_mode'));
        $allowed_access_modes = array('subscription_only', 'subscription_and_purchase', 'purchase_only');

        if (!in_array($access_mode, $allowed_access_modes, true)) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Please choose a valid access mode.',
            );
        ...
        if ($access_mode == 'subscription_only') {
            return array(
                'success' => true,
                'data' => array(
                    'youngo_access_mode' => 'subscription_only',
                    'youngo_allow_individual_purchase' => 0,
                    'youngo_purchase_access_type' => 'lifetime',
                    'youngo_purchase_duration_days' => null,
                    'youngo_subscription_excluded' => 0,
                ),
            );
        }

        if (!$this->youngo_post_value_is_scalar('youngo_purchase_access_type')) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Please choose a valid purchase access type.',
            );
        }

        $purchase_access_type = trim((string) $this->input->post('youngo_purchase_access_type'));
        if (!in_array($purchase_access_type, array('lifetime', 'timed'), true)) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Please choose a valid purchase access type.',
            );
        ...
        if (!$this->youngo_post_value_is_scalar('is_free_course')) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Disable legacy free-course mode and provide valid pricing before saving a purchase-enabled access mode.',
            );
        }

        if ($this->youngo_post_value_is_one('is_free_course')) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Disable legacy free-course mode and provide valid pricing before saving a purchase-enabled access mode.',
            );
        ...
        if (!$this->youngo_is_positive_numeric_value($price)) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Provide a valid course price greater than zero before saving a purchase-enabled access mode.',
            );
        }

        if (!$this->youngo_post_value_is_scalar('discount_flag')) {
            return array(
                'success' => false,
                'message' => 'YounGo access settings could not be saved. Provide valid pricing before saving a purchase-enabled access mode.',
            );
        }

        if ($this->youngo_post_value_is_one('discount_flag')) {
            $discounted_price = $this->input->post('discounted_price');
            if (!$this->youngo_is_positive_numeric_value($discounted_price) || (float) $discounted_price >= (float) $price) {
                return array(
                    'success' => false,
                    'message' => 'YounGo access settings could not be saved. Provide a valid discounted price lower than the course price.',
                );
            }
        }

        $duration_days = null;
        if ($purchase_access_type == 'timed') {
            if (!$this->youngo_post_value_is_scalar('youngo_purchase_duration_days')) {
                return array(
                    'success' => false,
                    'message' => 'YounGo access settings could not be saved. Timed purchase access requires a positive duration in days.',
                );
            }

            $duration_days = trim((string) $this->input->post('youngo_purchase_duration_days'));
            if (!preg_match('/^[1-9][0-9]*$/', $duration_days)) {
                return array(
                    'success' => false,
                    'message' => 'YounGo access settings could not be saved. Timed purchase access requires a positive duration in days.',
                );
        ...
        return array(
            'success' => true,
            'data' => array(
                'youngo_access_mode' => $access_mode,
                'youngo_allow_individual_purchase' => 1,
                'youngo_purchase_access_type' => $purchase_access_type,
                'youngo_purchase_duration_days' => $duration_days,
                'youngo_subscription_excluded' => $access_mode == 'purchase_only' ? 1 : 0,
            ),
        );
    }

    private function youngo_post_value_is_scalar($field)
    {
        $value = $this->input->post($field);
        return $value === null || is_scalar($value);
    }

    private function youngo_post_value_is_one($field)
    {
        $value = $this->input->post($field);
        return is_scalar($value) && (string) $value === '1';
    }

    private function youngo_is_positive_numeric_value($value)
    {
        return is_scalar($value) && is_numeric($value) && (float) $value > 0;
    }

    public function update_course($course_id, $type = "", $options = array())
    {
        if (!is_array($options)) {
            $options = array();
        }
        ...
        $outcomes = $this->trim_and_return_json($this->input->post('outcomes'));
        $requirements = $this->trim_and_return_json($this->input->post('requirements'));
        $data['title'] = $course_translation_fields['english']['title'];
        $data['short_description'] = html_escape($course_translation_fields['english']['short_description']);
        $data['description'] = $course_translation_fields['english']['description'];
        $data['outcomes'] = $course_translation_fields['english']['outcomes'];
        $data['faqs'] = $course_translation_fields['english']['faqs'];
        ...
        if ($pricing_fields_posted) {
            ...
            $data['price'] = isset($course_details['price']) ? $course_details['price'] : 0;
            $data['discount_flag'] = isset($course_details['discount_flag']) ? $course_details['discount_flag'] : 0;
            $data['discounted_price'] = isset($course_details['discounted_price']) ? $course_details['discounted_price'] : 0;
        }
        $data['level'] = $this->input->post('level');
    ...
    {
        $translation_fields = $this->get_posted_section_translation_fields();
        ...
        $data['title'] = html_escape($translation_fields['english_title']);
        $data['course_id'] = $course_id;
        $this->db->insert('section', $data);
        $section_id = $this->db->insert_id();
        $this->sync_section_translations($section_id, $translation_fields);
    ...
    public function edit_section($section_id)
    {
        ...
        $data['title'] = $translation_fields['english_title'];
    ...
    public function add_lesson()
    {
        ...
        $translation_fields = $this->get_posted_lesson_translation_fields($lesson_type, $attachment_type);
        $this->sync_legacy_lesson_post_fields($translation_fields);
        $data['title'] = html_escape($translation_fields['english']['title']);
        ...
        if ($lesson_type == 'video') {
            ...
            try {
                ...
                $result = $s3->putObject([
                    'Bucket' => $bucket,
                    'Key' => slugify(get_settings('system_name')) . '/' . slugify($course['title']) . '/' . slugify($translation_fields['english']['title']) . '-' . time() . '.' . $ext, // Desired file name on Wasabi
            ...
            $data['attachment'] = htmlspecialchars_($translation_fields['english']['text_content']);
        } else {
        ...
        $data['date_added'] = strtotime(date('D, d-M-Y'));
        $data['summary'] = htmlspecialchars_($translation_fields['english']['summary']);
        ...
        $this->sync_lesson_translations($inserted_id, $translation_fields);
    ...
    public function edit_lesson($lesson_id)
    {
        ...
        // Form values may already contain HTML entities from an older save.
        // Decode them before writing so repeated edits never turn & into &amp;amp;.
        $clean_lesson_url = static function ($value) {
            return trim(htmlspecialchars_decode((string) $value, ENT_QUOTES));
        };

        $data['course_id'] = html_escape($this->input->post('course_id') !== null ? $this->input->post('course_id') : $previous_data['course_id']);
        $data['section_id'] = html_escape($this->input->post('section_id') !== null ? $this->input->post('section_id') : $previous_data['section_id']);

        $posted_lesson_type = $this->input->post('lesson_type');
        $lesson_type_array = explode('-', $posted_lesson_type !== null && $posted_lesson_type !== '' ? $posted_lesson_type : $previous_data['lesson_type'] . '-' . $previous_data['attachment_type']);
        ...
        if ($lesson_type == 'video') {
            ...
            if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
                ...
                $data['video_url'] = $clean_lesson_url($this->input->post('video_url'));
                ...
                $data['video_url'] = $clean_lesson_url($this->input->post('html5_video_url'));
                ...
                $data['video_url'] = $clean_lesson_url($this->input->post('google_drive_video_url'));
                $duration_formatter = explode(':', $this->input->post('google_drive_video_duration'));
                $hour = sprintf('%02d', $duration_formatter[0]);
                $min = sprintf('%02d', $duration_formatter[1]);
                $sec = sprintf('%02d', $duration_formatter[2]);
            ...
            $data['duration_for_mobile_application'] = $hour . ':' . $min . ':' . $sec;
            $data['video_type_for_mobile_application'] = 'html5';
            $data['video_url_for_mobile_application'] = $clean_lesson_url($mobile_app_lesson_url);
        } elseif ($lesson_type == "s3") {
            // SET MAXIMUM EXECUTION TIME 600
            ini_set('max_execution_time', '600');
            ...
            if ($attachment_type == 'audio') {
                if (isset($_FILES['system_audio_file']) && !empty($_FILES['system_audio_file']['name'])) {
        ...
        $this->sync_lesson_translations($lesson_id, $translation_fields);
    ...
    public function get_total_duration_of_lesson_by_course_id($course_id)
{
    ...
    function get_application_details()
    {
        $purchase_code = get_settings('purchase_code');
            ...
            'support_expiry_date'  => date("d M, Y", strtotime("+5 year")),
            'customer_name'        => 'Valid',
    ...
    {
        if (isset($_POST['code']) && !empty($_POST['code']) && isset($_POST['discount_percentage']) && is_numeric($_POST['discount_percentage']) && isset($_POST['start_date']) && !empty($_POST['start_date']) && isset($_POST['expiry_date']) && !empty($_POST['expiry_date'])) {
            $start_date = strtotime($this->input->post('start_date'));
            $expiry_date = strtotime($this->input->post('expiry_date'));
            $percentage = (float) $this->input->post('discount_percentage');
            if ($start_date === false || $expiry_date === false || $start_date > $expiry_date || $percentage <= 0 || $percentage > 100) return false;
            $data['code'] = strtoupper(trim($this->input->post('code')));
            $data['discount_percentage'] = $percentage;
            $data['discount_type'] = 'percentage';
            $data['discount_value'] = $percentage;
            $data['start_date'] = $start_date;
            $data['expiry_date'] = $expiry_date;
            $data['status'] = $this->input->post('status') === 'inactive' ? 'inactive' : 'active';
    ...
    public function check_coupon_validity($coupon_code)
    {
        ...
        if ($result->num_rows() > 0) {
            ...
            if (($result['status'] ?? 'active') === 'active' && (!isset($result['start_date']) || !$result['start_date'] || $result['start_date'] <= time()) && $result['expiry_date'] >= strtotime(date('D, d-M-Y'))) {
                return true;
            } else {
                return false;
            }
    ...
    {
        $posted_translations = $this->youngo_blog_category_posted_translation_fields();
        $data['title'] = htmlspecialchars_($posted_translations['english']['title']);
        $data['subtitle'] = htmlspecialchars_($posted_translations['english']['subtitle']);
        ...
        if ($row->num_rows() > 0) {
            ...
            $this->youngo_save_blog_category_translations_from_post($this->db->insert_id(), $posted_translations);
            return true;
        }
    }
    ...
    function update_blog_category($blog_category_id = "")
    {
        ...
        if ($row->num_rows() > 0 && $row->row('blog_category_id') != $blog_category_id) {
            ...
            $this->youngo_save_blog_category_translations_from_post($blog_category_id, $posted_translations);
            return true;
        }
    }
    ...
    {
        if ($this->db->table_exists('youngo_blog_category_translations')) {
            $this->db->where('blog_category_id', $blog_category_id);
            $this->db->delete('youngo_blog_category_translations');
        }
    ...
    function youngo_normalize_blog_category_translation_language($language_code = 'english')
    {
        ...
        if ($language_code === 'english' || $language_code === 'arabic') {
    ...
    function youngo_get_blog_category_translation($blog_category_id = 0, $language_code = 'english')
    {
        $blog_category_id = (int) $blog_category_id;
        $language_code = $this->youngo_normalize_blog_category_translation_language($language_code);

        if ($blog_category_id <= 0 || $language_code === false || ! $this->db->table_exists('youngo_blog_category_translations')) {
            return array();
        }

        $this->db->where('blog_category_id', $blog_category_id);
        $this->db->where('language_code', $language_code);
        $row = $this->db->get('youngo_blog_category_translations');

        return $row->num_rows() > 0 ? $row->row_array() : array();
    }

    function youngo_get_blog_category_translations($blog_category_id = 0)
    {
        $blog_category_id = (int) $blog_category_id;
        $translations = array('english' => array(), 'arabic' => array());

        if ($blog_category_id <= 0 || ! $this->db->table_exists('youngo_blog_category_translations')) {
            return $translations;
        }

        $this->db->where('blog_category_id', $blog_category_id);
        $this->db->where_in('language_code', array('english', 'arabic'));
        $rows = $this->db->get('youngo_blog_category_translations')->result_array();
    ...
    function youngo_get_blog_category_translations_by_ids($blog_category_ids = array(), $language_code = 'english')
    {
        $language_code = $this->youngo_normalize_blog_category_translation_language($language_code);
        if ($language_code === false || ! $this->db->table_exists('youngo_blog_category_translations')) {
            return array();
        }

        $ids = array();
        foreach ((array) $blog_category_ids as $blog_category_id) {
            $blog_category_id = (int) $blog_category_id;
            if ($blog_category_id > 0) {
                $ids[] = $blog_category_id;
            }
        }
        ...
        $translations = array();
        foreach ($rows as $row) {
            $translations[(int) $row['blog_category_id']] = $row;
        }

        return $translations;
    }

    function youngo_apply_blog_category_translation($category = array(), $language_code = 'english')
    {
        if (empty($category) || !is_array($category)) {
            return $category;
        }

        $blog_category_id = isset($category['blog_category_id']) ? (int) $category['blog_category_id'] : 0;
        $language_code = $this->youngo_normalize_blog_category_translation_language($language_code);
        $language_code = $language_code === false ? 'english' : $language_code;
        $canonical_title = isset($category['title']) ? $category['title'] : '';
        $canonical_subtitle = isset($category['subtitle']) ? $category['subtitle'] : '';
        $canonical_slug = isset($category['slug']) ? $category['slug'] : '';
        $requested_translation = $this->youngo_get_blog_category_translation($blog_category_id, $language_code);
        $english_translation = array();

        if ($language_code === 'arabic') {
            $english_translation = $this->youngo_get_blog_category_translation($blog_category_id, 'english');
        }

        $category['youngo_canonical_title'] = $canonical_title;
        $category['youngo_canonical_subtitle'] = $canonical_subtitle;
        $category['youngo_canonical_slug'] = $canonical_slug;
        $category['youngo_translation_requested_language'] = $language_code;
        $category['youngo_translation_source'] = 'blog_category';
        $category['youngo_translation_is_fallback'] = true;

        foreach (array('title', 'subtitle') as $field) {
            if (isset($requested_translation[$field]) && trim((string) $requested_translation[$field]) !== '') {
                $category[$field] = $requested_translation[$field];
                $category['youngo_translation_source'] = 'youngo_blog_category_translations';
                $category['youngo_translation_is_fallback'] = false;
                continue;
            }

            if ($language_code === 'arabic' && isset($english_translation[$field]) && trim((string) $english_translation[$field]) !== '') {
                $category[$field] = $english_translation[$field];
                $category['youngo_translation_source'] = 'youngo_blog_category_translations';
                continue;
            }
        }
        ...
        $category['youngo_display_slug'] = isset($requested_translation['display_slug']) && trim((string) $requested_translation['display_slug']) !== ''
            ? $requested_translation['display_slug']
            : $canonical_slug;

        return $category;
    }

    function youngo_apply_blog_category_translations($categories = array(), $language_code = 'english')
    {
        $localized = array();

        foreach ((array) $categories as $category) {
            $localized[] = $this->youngo_apply_blog_category_translation($category, $language_code);
        }

        return $localized;
    }

    function youngo_save_blog_category_translation($blog_category_id = 0, $language_code = 'english', $fields = array())
    {
        $blog_category_id = (int) $blog_category_id;
        $language_code = $this->youngo_normalize_blog_category_translation_language($language_code);
        ...
        $subtitle = isset($fields['subtitle']) ? trim((string) $fields['subtitle']) : '';
        $display_slug = isset($fields['display_slug']) ? trim((string) $fields['display_slug']) : '';

        if ($title === '' && $subtitle === '' && $display_slug === '') {
            $this->db->where('blog_category_id', $blog_category_id);
            $this->db->where('language_code', $language_code);
            $this->db->delete('youngo_blog_category_translations');
            return true;
        ...
        if ($title === '') {
            $category = $this->get_blog_categories($blog_category_id)->row_array();
            $title = isset($category['title']) ? $category['title'] : 'YounGo';
        }

        $now = time();
        $user_id = $this->session->userdata('user_id') ? (int) $this->session->userdata('user_id') : null;
        $data = array(
            'blog_category_id' => $blog_category_id,
            'language_code' => $language_code,
            'title' => htmlspecialchars_($title),
            'subtitle' => htmlspecialchars_($subtitle),
            'display_slug' => $display_slug === '' ? null : htmlspecialchars_($display_slug),
        ...
        $this->db->where('blog_category_id', $blog_category_id);
        $this->db->where('language_code', $language_code);
        $existing = $this->db->get('youngo_blog_category_translations');

        if ($existing->num_rows() > 0) {
            $this->db->where('id', $existing->row('id'));
            $this->db->update('youngo_blog_category_translations', $data);
        } else {
            $data['created_by_user_id'] = $user_id;
            $data['created_at'] = $now;
            $this->db->insert('youngo_blog_category_translations', $data);
        }

        return true;
    }

    function youngo_save_blog_category_translations_from_post($blog_category_id = 0, $posted_translations = null)
    {
        if (!is_array($posted_translations)) {
            $posted_translations = $this->youngo_blog_category_posted_translation_fields();
        }

        $this->youngo_save_blog_category_translation($blog_category_id, 'english', $posted_translations['english']);
        $this->youngo_save_blog_category_translation($blog_category_id, 'arabic', $posted_translations['arabic']);
    }

    function youngo_blog_category_posted_translation_fields()
    {
        $english_title = $this->input->post('english_title');
        if ($english_title === null || trim((string) $english_title) === '') {
            $english_title = $this->input->post('title');
        }

        $english_subtitle = $this->input->post('english_subtitle');
        if ($english_subtitle === null) {
            $english_subtitle = $this->input->post('subtitle');
        }

        return array(
            'english' => array(
                'title' => trim((string) $english_title),
                'subtitle' => trim((string) $english_subtitle),
                'display_slug' => trim((string) $this->input->post('english_display_slug')),
            ),
            'arabic' => array(
                'title' => trim((string) $this->input->post('arabic_title')),
                'subtitle' => trim((string) $this->input->post('arabic_subtitle')),
                'display_slug' => trim((string) $this->input->post('arabic_display_slug')),
            ),
        );
    }
    ...
    function youngo_normalize_blog_translation_language($language_code = 'english')
    {
        return $language_code === 'arabic' ? 'arabic' : 'english';
    }

    function youngo_get_blog_translation($blog_id = 0, $language_code = 'english')
    {
        $blog_id = (int) $blog_id;
        if ($blog_id <= 0 || ! $this->db->table_exists('youngo_blog_translations')) {
            return array();
        }

        $this->db->where('blog_id', $blog_id);
        $this->db->where('language_code', $this->youngo_normalize_blog_translation_language($language_code));
        $row = $this->db->get('youngo_blog_translations');

        return $row->num_rows() > 0 ? $row->row_array() : array();
    }

    function youngo_save_blog_translation($blog_id = 0, $language_code = 'english', $title = '', $excerpt = '', $description = '')
    {
        $blog_id = (int) $blog_id;
        if ($blog_id <= 0 || ! $this->db->table_exists('youngo_blog_translations')) {
            return false;
        ...
        $language_code = $this->youngo_normalize_blog_translation_language($language_code);
        ...
        if ($title === '') {
            ...
            $title = isset($blog['title']) ? $blog['title'] : 'YounGo Blog';
        }
        ...
        $data = array(
            ...
            'language_code' => $language_code,
            'title' => htmlspecialchars_($title),
            'excerpt' => htmlspecialchars_($excerpt),
            'description' => htmlspecialchars_(remove_js($description)),
            'slug' => slugify($title),
            'updated_at' => $now,
        );
        ...
        $this->db->where('blog_id', $blog_id);
        $this->db->where('language_code', $language_code);
        $existing = $this->db->get('youngo_blog_translations');

        if ($existing->num_rows() > 0) {
            $this->db->where('id', $existing->row('id'));
            $this->db->update('youngo_blog_translations', $data);
        } else {
            $data['created_at'] = $now;
            $this->db->insert('youngo_blog_translations', $data);
        }

        return true;
    }

    function youngo_save_blog_translations_from_post($blog_id = 0)
    {
        $english_title = $this->input->post('title');
        $english_excerpt = $this->input->post('english_excerpt');
        $english_description = $this->input->post('description', false);

        $this->youngo_save_blog_translation($blog_id, 'english', $english_title, $english_excerpt, $english_description);

        $arabic_title = $this->input->post('arabic_title');
        $arabic_excerpt = $this->input->post('arabic_excerpt');
        $arabic_description = $this->input->post('arabic_description', false);

        if (trim((string) $arabic_title) !== '' || trim((string) $arabic_excerpt) !== '' || trim((string) $arabic_description) !== '') {
            $this->youngo_save_blog_translation($blog_id, 'arabic', $arabic_title, $arabic_excerpt, $arabic_description);
        }
    }
    ...
    function add_blog()
    {
        ...
        $this->youngo_save_blog_translations_from_post($this->db->insert_id());
    }
    ...
    function update_blog($blog_id = "")
    {
        ...
        $this->youngo_save_blog_translations_from_post($blog_id);
    }
    ...
    function youngo_homepage_sanitize_cta($cta, $default = array())
    {
        $default = is_array($default) ? $default : array('label' => '', 'url' => '');
        $cta = is_array($cta) ? $cta : array();

        return array(
            'label' => youngo_homepage_plain_text(isset($cta['label']) ? $cta['label'] : (isset($default['label']) ? $default['label'] : '')),
            'url' => youngo_homepage_safe_url(isset($cta['url']) ? $cta['url'] : (isset($default['url']) ? $default['url'] : ''), isset($default['url']) ? $default['url'] : ''),
        );
    }

    function youngo_homepage_sanitize_image($image, $default = array())
    {
        $default = is_array($default) ? $default : array('url' => '', 'alt' => '');
        $image = is_array($image) ? $image : array();

        return array(
            'url' => youngo_homepage_safe_image_path(isset($image['url']) ? $image['url'] : (isset($default['url']) ? $default['url'] : ''), isset($default['url']) ? $default['url'] : ''),
            'alt' => youngo_homepage_plain_text(isset($image['alt']) ? $image['alt'] : (isset($default['alt']) ? $default['alt'] : '')),
        );
    }

    function youngo_homepage_sanitize_items($items, $fields, $default_items = array(), $limit = 6)
    {
        ...
        foreach ($items as $item) {
            ...
            foreach ($fields as $field) {
                if ($field === 'rating') {
                    $row[$field] = youngo_homepage_normalize_int(isset($item[$field]) ? $item[$field] : 5, 5, 0, 5);
                    continue;
                }
                if ($field === 'image') {
                    $row[$field] = $this->youngo_homepage_sanitize_image(isset($item[$field]) ? $item[$field] : array());
                    continue;
                }
                $value = youngo_homepage_plain_text(isset($item[$field]) ? $item[$field] : '');
                $row[$field] = $value;
                if ($value !== '') {
                    $has_content = true;
                }
        ...
        if (count($sanitized) === 0) {
            $sanitized = $default_items;
        }

        return array_slice($sanitized, 0, $limit);
    }

    function youngo_homepage_sanitize_text_list($items, $default_items = array(), $limit = 6)
    {
        $items = is_array($items) ? $items : array();
        $sanitized = array();

        foreach ($items as $item) {
            $value = youngo_homepage_plain_text($item);
            if ($value !== '') {
                $sanitized[] = $value;
            }
        }
        ...
        if (count($sanitized) === 0) {
            $sanitized = is_array($default_items) ? $default_items : array();
        }

        return array_slice($sanitized, 0, $limit);
    }

    function youngo_homepage_sanitize_category_overrides($items, $default_items = array())
    {
        $items = is_array($items) ? $items : array();
        $sanitized = array();
        ...
        foreach ($items as $item) {
            ...
            $category_id = youngo_homepage_normalize_int(isset($item['category_id']) ? $item['category_id'] : 0, 0, 1, 999999);
            if ($category_id <= 0) {
                continue;
            }

            $image = isset($item['image']) && is_array($item['image']) ? $item['image'] : array();
            $description = youngo_homepage_plain_text(isset($item['description']) ? $item['description'] : '');
            $image = $this->youngo_homepage_sanitize_image($image);

            if ($description === '' && empty($image['url']) && empty($image['alt'])) {
                continue;
            }
        ...
        return count($sanitized) > 0 ? array_slice($sanitized, 0, 12) : (is_array($default_items) ? $default_items : array());
    }

    function youngo_homepage_sanitize_course_overrides($items, $default_items = array())
    {
        $items = is_array($items) ? $items : array();
        $sanitized = array();
        ...
        foreach ($items as $item) {
            ...
            $course_id = youngo_homepage_normalize_int(isset($item['course_id']) ? $item['course_id'] : 0, 0, 1, 999999);
            if ($course_id <= 0) {
                continue;
            }

            $image = isset($item['image']) && is_array($item['image']) ? $item['image'] : array();
            $badge = youngo_homepage_plain_text(isset($item['badge']) ? $item['badge'] : '');
            $image = $this->youngo_homepage_sanitize_image($image);

            if ($badge === '' && empty($image['url']) && empty($image['alt'])) {
                continue;
            }
    ...
    function youngo_homepage_sanitize_fixed_section_content($key, $posted_content, $default_content)
    {
        $posted_content = is_array($posted_content) ? $posted_content : array();
        $content = $default_content;

        if ($key === 'hero') {
            $content['eyebrow'] = youngo_homepage_plain_text(isset($posted_content['eyebrow']) ? $posted_content['eyebrow'] : $content['eyebrow']);
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : $content['title']);
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : $content['subtitle']);
            $content['primary_cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['primary_cta']) ? $posted_content['primary_cta'] : array(), $content['primary_cta']);
            $content['secondary_cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['secondary_cta']) ? $posted_content['secondary_cta'] : array(), $content['secondary_cta']);
            $content['image'] = $this->youngo_homepage_sanitize_image(isset($posted_content['image']) ? $posted_content['image'] : array(), $content['image']);
            $content['trust_items'] = $this->youngo_homepage_sanitize_text_list(isset($posted_content['trust_items']) ? $posted_content['trust_items'] : array(), isset($content['trust_items']) ? $content['trust_items'] : array(), 6);
            $content['stats'] = $this->youngo_homepage_sanitize_items(isset($posted_content['stats']) ? $posted_content['stats'] : array(), array('value', 'label'), isset($content['stats']) ? $content['stats'] : array(), 4);
            return $content;
        }

        if ($key === 'featured_categories' || $key === 'featured_courses') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : $content['title']);
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : $content['subtitle']);
            $content['limit'] = youngo_homepage_normalize_int(isset($posted_content['limit']) ? $posted_content['limit'] : $content['limit'], $content['limit'], 1, 12);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
            if ($key === 'featured_categories') {
                $content['category_ids'] = youngo_homepage_normalize_id_list(isset($posted_content['category_ids']) ? $posted_content['category_ids'] : (isset($content['category_ids']) ? $content['category_ids'] : array()));
                $content['overrides'] = $this->youngo_homepage_sanitize_category_overrides(isset($posted_content['overrides']) ? $posted_content['overrides'] : array(), isset($content['overrides']) ? $content['overrides'] : array());
            } else {
                $content['course_ids'] = youngo_homepage_normalize_id_list(isset($posted_content['course_ids']) ? $posted_content['course_ids'] : (isset($content['course_ids']) ? $content['course_ids'] : array()));
                $content['category_ids'] = youngo_homepage_normalize_id_list(isset($posted_content['category_ids']) ? $posted_content['category_ids'] : (isset($content['category_ids']) ? $content['category_ids'] : array()));
                $content['overrides'] = $this->youngo_homepage_sanitize_course_overrides(isset($posted_content['overrides']) ? $posted_content['overrides'] : array(), isset($content['overrides']) ? $content['overrides'] : array());
            }
            return $content;
        }
        ...
        if ($key === 'why_choose') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : $content['title']);
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : $content['subtitle']);
            $content['items'] = $this->youngo_homepage_sanitize_items(isset($posted_content['items']) ? $posted_content['items'] : array(), array('icon_key', 'title', 'description'), $content['items'], 6);
            return $content;
        }

        if ($key === 'about_teaser') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : $content['title']);
            $content['body'] = youngo_homepage_plain_text(isset($posted_content['body']) ? $posted_content['body'] : $content['body']);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
            $content['image'] = $this->youngo_homepage_sanitize_image(isset($posted_content['image']) ? $posted_content['image'] : array(), isset($content['image']) ? $content['image'] : array());
            $content['stats'] = $this->youngo_homepage_sanitize_items(isset($posted_content['stats']) ? $posted_content['stats'] : array(), array('value', 'label'), isset($content['stats']) ? $content['stats'] : array(), 4);
            $content['items'] = array_values(array_filter(array_map('youngo_homepage_plain_text', isset($posted_content['items']) && is_array($posted_content['items']) ? $posted_content['items'] : $content['items'])));
            return $content;
        }
        ...
        if ($key === 'testimonials') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : $content['title']);
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : $content['subtitle']);
            $content['items'] = $this->youngo_homepage_sanitize_items(isset($posted_content['items']) ? $posted_content['items'] : array(), array('quote', 'name', 'role', 'rating'), $content['items'], 6);
            return $content;
        }
        ...
        if ($key === 'faq_preview') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : $content['title']);
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : $content['subtitle']);
            $content['limit'] = youngo_homepage_normalize_int(isset($posted_content['limit']) ? $posted_content['limit'] : $content['limit'], $content['limit'], 1, 10);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
            $content['items'] = $this->youngo_homepage_sanitize_items(isset($posted_content['items']) ? $posted_content['items'] : array(), array('question', 'answer'), $content['items'], 10);
            return $content;
        }
        ...
        if ($key === 'blog_preview') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : $content['title']);
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : $content['subtitle']);
            $content['limit'] = youngo_homepage_normalize_int(isset($posted_content['limit']) ? $posted_content['limit'] : $content['limit'], $content['limit'], 1, 9);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
            return $content;
        }
    ...
    function youngo_homepage_sanitize_additional_content($type, $posted_content)
    {
        $posted_content = is_array($posted_content) ? $posted_content : array();
        $content = youngo_homepage_additional_content_default($type);

        if ($type === 'text_image') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : '');
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : '');
            $content['body'] = youngo_homepage_plain_text(isset($posted_content['body']) ? $posted_content['body'] : '');
            $content['image'] = $this->youngo_homepage_sanitize_image(isset($posted_content['image']) ? $posted_content['image'] : array(), $content['image']);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
        } elseif ($type === 'feature_cards') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : '');
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : '');
            $content['items'] = $this->youngo_homepage_sanitize_items(isset($posted_content['items']) ? $posted_content['items'] : array(), array('icon_key', 'title', 'description'), array(), 6);
        } elseif ($type === 'cta_band') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : '');
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : '');
            $content['primary_cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['primary_cta']) ? $posted_content['primary_cta'] : array(), $content['primary_cta']);
            $content['secondary_cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['secondary_cta']) ? $posted_content['secondary_cta'] : array(), $content['secondary_cta']);
        } elseif ($type === 'testimonial_block') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : '');
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : '');
            $content['items'] = $this->youngo_homepage_sanitize_items(isset($posted_content['items']) ? $posted_content['items'] : array(), array('name', 'role', 'quote', 'rating', 'image'), array(), 6);
        } elseif ($type === 'faq_block') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : '');
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : '');
            $content['items'] = $this->youngo_homepage_sanitize_items(isset($posted_content['items']) ? $posted_content['items'] : array(), array('question', 'answer'), array(), 10);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
        } elseif ($type === 'course_highlight') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : '');
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : '');
            $content['course_ids'] = youngo_homepage_normalize_id_list(isset($posted_content['course_ids']) ? $posted_content['course_ids'] : array());
            $content['limit'] = youngo_homepage_normalize_int(isset($posted_content['limit']) ? $posted_content['limit'] : 3, 3, 1, 12);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
        } elseif ($type === 'category_highlight') {
            $content['title'] = youngo_homepage_plain_text(isset($posted_content['title']) ? $posted_content['title'] : '');
            $content['subtitle'] = youngo_homepage_plain_text(isset($posted_content['subtitle']) ? $posted_content['subtitle'] : '');
            $content['category_ids'] = youngo_homepage_normalize_id_list(isset($posted_content['category_ids']) ? $posted_content['category_ids'] : array());
            $content['limit'] = youngo_homepage_normalize_int(isset($posted_content['limit']) ? $posted_content['limit'] : 4, 4, 1, 12);
            $content['cta'] = $this->youngo_homepage_sanitize_cta(isset($posted_content['cta']) ? $posted_content['cta'] : array(), $content['cta']);
        }
    ...
    function update_youngo_homepage_content()
    {
        $homepage = $this->input->post('homepage', false);
        $posted_sections = $this->input->post('sections', false);
        $posted_additional_sections = $this->input->post('additional_sections', false);

        $homepage = is_array($homepage) ? $homepage : array();
        $posted_sections = is_array($posted_sections) ? $posted_sections : array();
        $posted_additional_sections = is_array($posted_additional_sections) ? $posted_additional_sections : array();

        $content = youngo_homepage_default_content();
        $content['is_active'] = isset($homepage['is_active']) && youngo_homepage_normalize_bool($homepage['is_active']);
        $content['is_published'] = isset($homepage['is_published']) && youngo_homepage_normalize_bool($homepage['is_published']);
        $content['last_updated_at'] = date('c');

        foreach (youngo_homepage_fixed_section_keys() as $key) {
            $posted_section = isset($posted_sections[$key]) && is_array($posted_sections[$key]) ? $posted_sections[$key] : array();
            $default_section = $content['sections'][$key];

            $content['sections'][$key]['is_visible'] = isset($posted_section['is_visible']) && youngo_homepage_normalize_bool($posted_section['is_visible']);
            $content['sections'][$key]['is_published'] = isset($posted_section['is_published']) && youngo_homepage_normalize_bool($posted_section['is_published']);
            $content['sections'][$key]['sort_order'] = youngo_homepage_normalize_int(isset($posted_section['sort_order']) ? $posted_section['sort_order'] : $default_section['sort_order'], $default_section['sort_order'], 0, 999);
            $content['sections'][$key]['source_type'] = youngo_homepage_normalize_source_type(isset($posted_section['source_type']) ? $posted_section['source_type'] : $default_section['source_type'], $default_section['source_type']);
            $content['sections'][$key]['content'] = $this->youngo_homepage_sanitize_fixed_section_content($key, isset($posted_section['content']) ? $posted_section['content'] : array(), $default_section['content']);
        }

        $additional_sections = array();
        foreach ($posted_additional_sections as $index => $section) {
            if (!is_array($section) || !empty($section['remove'])) {
                continue;
            }

            $type = isset($section['type']) ? $section['type'] : '';
            if (!in_array($type, youngo_homepage_allowed_additional_section_types())) {
                continue;
            }

            $additional_sections[] = array(
                'id' => !empty($section['id']) ? youngo_homepage_plain_text($section['id']) : 'additional_' . ($index + 1) . '_' . time(),
                'type' => $type,
                'label' => youngo_homepage_plain_text(isset($section['label']) ? $section['label'] : ucwords(str_replace('_', ' ', $type))),
                'is_visible' => isset($section['is_visible']) && youngo_homepage_normalize_bool($section['is_visible']),
                'is_published' => isset($section['is_published']) && youngo_homepage_normalize_bool($section['is_published']),
                ...
                'content' => $this->youngo_homepage_sanitize_additional_content($type, isset($section['content']) ? $section['content'] : array()),
                'fallback' => array('use_default_if_empty' => false),
            );
        }
        $content['additional_sections'] = youngo_homepage_normalize_additional_sections($additional_sections);

        $data = array('value' => json_encode($content));
        $row = $this->db->where('key', 'youngo_homepage_content')->get('frontend_settings');
        if ($row->num_rows() > 0) {
            $this->db->where('key', 'youngo_homepage_content')->update('frontend_settings', $data);
        } else {
            $this->db->insert('frontend_settings', array('key' => 'youngo_homepage_content', 'value' => $data['value']));
        }
    }
